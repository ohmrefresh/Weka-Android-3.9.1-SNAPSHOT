 ...
 *  the type specification - e.g. "myTime&#64;date:MM-dd-yyyy&#64;08-23-2009".This option may be
 ...
 */
public class AddUserFields extends Filter
    implements OptionHandler, EnvironmentHandler, UnsupervisedFilter {
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option("\tNew field specification (name@type@value).\n"
        ...
        + "This option may be specified multiple times", "A", 1, "-A <name@type@value>"));
  ...
  public static class AttributeSpec implements Serializable {
    ...
    public String getNominalOrStringValue() {
      if (getResolvedType().toLowerCase().startsWith("nominal") || getResolvedType().toLowerCase()
          .startsWith("string")) {
      ...
      if (m_typeS.toLowerCase().startsWith("date") && m_typeS.indexOf(":") > 0) {
        ...
        if (!m_valueS.toLowerCase().equals("now")) {
          try {
            ...
            throw new IllegalArgumentException("Date value \""
                + m_valueS
                + " \" can't be parsed with formatting string \""
                + format
                + "\"");
          }
        }
