 ...
 */
public class MergeManyValues extends PotentialClassIgnorer
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tSets the attribute index\n" + "\t(default: last)", "C", 1, "-C <col>"));

    newVector.addElement(
        new Option("\tSets the label of the newly merged classes\n" + "\t(default: 'merged')", "L",
            1, "-L <label>"));
  ...
  @Override public boolean setInputFormat(Instances instanceInfo) throws Exception {
    ...
    if ((inputFormatPeek().classIndex() > -1) && (inputFormatPeek().classIndex()
        == m_AttIndex.getIndex())) {
