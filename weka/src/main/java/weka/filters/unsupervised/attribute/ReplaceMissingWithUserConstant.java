...
public class ReplaceMissingWithUserConstant extends PotentialClassIgnorer
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    opts.addElement(
        new Option("\tSpecify the replacement constant for nominal/string attributes", "N", 1,
            "-N"));
    opts.addElement(
        new Option("\tSpecify the replacement constant for numeric attributes" + "\n\t(default: 0)",
            "R", 1, "-R"));
    opts.addElement(
        new Option("\tSpecify the replacement constant for date attributes", "D", 1, "-D"));
  ...
  @Override public boolean setInputFormat(Instances instanceInfo) throws Exception {
    ...
    try {
      ...
      for (String att : parts) {
        ...
        if (a == null) {
          throw new Exception(
              "I can't find the requested attribute '" + att + "' in the incoming instances.");
        }
      ...
      if (m_numericConstant == null || m_numericConstant.length() == 0) {
        if (m_resolvedNominalStringConstant != null
          ...
          try {
            ...
            throw new Exception("Data contains numeric attributes and no numeric "
    ...
    if (hasDate) {
      if (m_resolvedDateConstant == null || m_resolvedDateConstant.length() == 0) {
        throw new Exception(
            "Data contains date attributes and no replacement constant has been " + "supplied");
      }
        ...
        if (i != inst.classIndex()) {
          if (inst.attribute(i).isDate()) {
            ...
            if (inst.attribute(i).numValues() <= 1) {
              ...
              vals[i] =
                  outputFormatPeek().attribute(i).addStringValue(m_resolvedNominalStringConstant);
            }
          } else {
            vals[i] = inst.value(i);
          }
