...
public class NumericCleaner extends SimpleStreamFilter {
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    result.addElement(new Option("\tThe minimum threshold. (default -Double.MAX_VALUE)", "min", 1,
        "-min <double>"));
    ...
    result.addElement(new Option("\tThe maximum threshold. (default Double.MAX_VALUE)", "max", 1,
        "-max <double>"));
    ...
    result.addElement(
        new Option("\tThe number values are checked for closeness. (default 0)", "closeto", 1,
            "-closeto <double>"));

    result.addElement(
        new Option("\tThe replacement for values that are close to '-closeto'.\n" + "\t(default 0)",
  ...
  @Override protected Instance process(Instance instance) throws Exception {
    ...
    for (i = 0; i < instance.numAttributes(); i++) {
      ...
      // too close?
      else if ((result[i] - m_CloseTo < m_CloseToTolerance) && (m_CloseTo - result[i]
          < m_CloseToTolerance) && (result[i] != m_CloseTo)) {
        if (getDebug()) {
