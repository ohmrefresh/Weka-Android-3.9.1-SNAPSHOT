 ...
 */
public class Normalize extends PotentialClassIgnorer
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    result.addElement(
        new Option("\tThe scaling factor for the output range.\n" + "\t(default: 1.0)", "S", 1,
            "-S <num>"));

    result.addElement(
        new Option("\tThe translation of the output range.\n" + "\t(default: 0.0)", "T", 1,
            "-T <num>"));
      ...
      for (int j = 0; j < instance.numAttributes(); j++) {
        ...
        if (instance.attribute(j).isNumeric()
          ...
          if (Double.isNaN(m_MinArray[j]) || (m_MaxArray[j] == m_MinArray[j])) {
            ...
            value = (vals[j] - m_MinArray[j]) / (m_MaxArray[j] - m_MinArray[j]) * m_Scale
                + m_Translation;
            if (Double.isNaN(value)) {
              throw new Exception(
                  "A NaN value was generated " + "while normalizing " + instance.attribute(j)
                      .name());
            }
          }
          if (value != 0.0) {
      ...
      for (int j = 0; j < getInputFormat().numAttributes(); j++) {
        if (instance.attribute(j).isNumeric()
          ...
          if (Double.isNaN(m_MinArray[j]) || (m_MaxArray[j] == m_MinArray[j])) {
            ...
            vals[j] = (vals[j] - m_MinArray[j]) / (m_MaxArray[j] - m_MinArray[j]) * m_Scale
                + m_Translation;
   ...
   */
  @Override public String toSource(String className, Instances data) throws Exception {
    StringBuffer result;
    boolean[] process;
    int i;
    ...
    result.append(
        "  protected final static double[] MIN = new double[]{" + Utils.arrayToString(m_MinArray)
            .replaceAll("NaN", "Double.NaN") + "};\n");
    result.append("\n");
