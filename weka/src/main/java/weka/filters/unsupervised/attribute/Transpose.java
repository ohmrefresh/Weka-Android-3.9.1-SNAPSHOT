...
/**
 * <!-- globalinfo-start -->
 * Transposes the data: instances become attributes and attributes become instances. If the first
 * attribute in the original data is a nominal or string identifier attribute, this identifier
 * attribute will be used to create attribute names in the transposed data. All attributes other
 * than the identifier attribute must be numeric. The attribute names in the original data are used
 * to create an identifier attribute of type string in the transposed data.<br/>
 * <br/>
 * This filter can only process one batch of data, e.g. it cannot be used in the the
 * FilteredClassifier.<br/>
 * <br/>
...
public class Transpose extends SimpleBatchFilter implements UnsupervisedFilter {
  ...
  @Override protected Instances process(Instances instances) throws Exception {
    ...
    // Transpose data
    double[][] newData =
        new double[instances.numAttributes() - offset][instances.numInstances() + 1];
    for (int i = 0; i < instances.numInstances(); i++) {
      for (int j = offset; j < instances.numAttributes(); j++) {
        newData[j - offset][0] =
            getOutputFormat().attribute(0).addStringValue(instances.attribute(j).name());
