...
/**
 * <!-- globalinfo-start -->
 * Replaces all missing values for nominal and numeric attributes in a dataset with the modes and
...
public class ReplaceMissingValues extends PotentialClassIgnorer
    ...
    if (instance instanceof SparseInstance) {
      ...
      for (int j = 0; j < instance.numAttributes(); j++) {
        if (instance.isMissing(j) &&
            ...
            (getInputFormat().attribute(j).isNominal() || getInputFormat().attribute(j)
                .isNumeric())) {
  ...
  public String toSource(String className, Instances data) throws Exception {
    ...
    result.append(
        "  protected final static double[] MEANS = new double[]{" + Utils.arrayToString(means)
