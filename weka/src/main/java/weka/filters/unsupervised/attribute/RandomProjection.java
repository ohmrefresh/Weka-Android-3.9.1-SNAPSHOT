...
/**
 * <!-- globalinfo-start -->
 * Reduces the dimensionality of the data by projecting it onto a lower dimensional subspace using a
 * random matrix with columns of unit length (i.e. It will reduce the number of attributes in the
 * data while preserving much of its variation like PCA, but at a much less computational
 * cost).<br/>
 * It first applies the  NominalToBinary filter to convert all attributes to numeric before reducing
 * the dimension. It preserves the class attribute.<br/>
 * <br/>
 * For more information, see:<br/>
 * <br/>
 * Dmitriy Fradkin, David Madigan: Experiments with random projections for machine learning. In: KDD
 ...
 *    booktitle = {KDD '03: Proceedings of the ninth ACM SIGKDD international conference on
 ...
 *          Kibriya)]
 */
public class RandomProjection extends Filter
  ...
  public static final Tag[] TAGS_DSTRS_TYPE = {
      new Tag(SPARSE1, "Sparse1"), new Tag(SPARSE2, "Sparse2"), new Tag(GAUSSIAN, "Gaussian"),
  };
  /** for serialization */
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option(
        "\tThe number of dimensions (attributes) the data should be reduced to\n"
            + "\t(default 10; exclusive of the class attribute, if it is set).", "N", 1,
        "-N <number>"));
    ...
    newVector.addElement(
        new Option("\tReplace missing values using the ReplaceMissingValues filter", "M", 0, "-M"));
  ...
  @Override public boolean setInputFormat(Instances instanceInfo) throws Exception {
    ...
    if (m_replaceMissing != null) {
      ...
      temp = m_replaceMissing.setInputFormat(instanceInfo);
    }
  ...
  protected void setOutputFormat() {
    ...
    if (currentFormat.classIndex() != -1) { // if classindex is set
      ...
      attributes.add((Attribute) currentFormat.attribute(currentFormat.classIndex()).copy());
  ...
  protected Instance convertInstance(Instance currentInstance) {
    ...
    int classIndex =
        (m_ntob == null) ? getInputFormat().classIndex() : m_ntob.getOutputFormat().classIndex();
