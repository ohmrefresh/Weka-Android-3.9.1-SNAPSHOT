...
public class RemoveUseless extends Filter implements UnsupervisedFilter, OptionHandler {
      ...
      for (int i = 0; i < toFilter.numAttributes(); i++) {
        ...
        if (stats.missingCount == toFilter.numInstances()) {
          ...
          double variancePercent =
              (double) stats.distinctCount / (double) (stats.totalCount - stats.missingCount)
                  * 100.0;
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option("\tMaximum variance percentage allowed (default 99)", "M", 1,
        "-M <max variance %>"));
  ...
  public String maximumVariancePercentageAllowedTipText() {
    ...
    return
        "Set the threshold for the highest variance allowed before a nominal attribute will be deleted."
