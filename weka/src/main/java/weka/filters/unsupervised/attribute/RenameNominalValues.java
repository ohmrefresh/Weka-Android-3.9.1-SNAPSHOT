 ...
 */
public class RenameNominalValues extends Filter
  ...
  @Override public boolean setInputFormat(Instances instanceInfo) throws Exception {
    ...
    if (m_renameVals != null && m_renameVals.length() > 0) {
      ...
      for (String val : vals) {
        ...
        m_renameMap.put(m_ignoreCase ? parts[0].toLowerCase().trim() : parts[0].trim(),
            parts[1].trim());
      }
    }
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option(
        "\tInvert matching sense (i.e. act on all attributes other than those specified)", "V", 0,
        "-V"));
