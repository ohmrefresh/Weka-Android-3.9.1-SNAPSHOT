...
/**
 * <!-- globalinfo-start -->
 * Converts String attributes into a set of attributes representing word occurrence (depending on
 * the tokenizer) information from the text contained in the strings. The set of words (attributes)
...
public class StringToWordVector extends Filter implements UnsupervisedFilter, OptionHandler {
  ...
  public static final Tag[] TAGS_FILTER = {
      new Tag(FILTER_NONE, "No normalization"), new Tag(FILTER_NORMALIZE_ALL, "Normalize all data"),
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    result.addElement(new Option(
        ...
        "dictionary", 1, "-dictionary <path to save to>"));
  ...
  @Override public boolean batchFinished() throws Exception {
    ...
    if (!isFirstBatchDone()) {
      ...
      Instances converted =
          m_dictionaryBuilder.vectorizeBatch(getInputFormat(), m_filterType != FILTER_NONE);
