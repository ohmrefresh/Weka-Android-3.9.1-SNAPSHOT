...
public class CheckSource implements OptionHandler, RevisionHandler {
      ...
      for (i = 0; i < data.numInstances(); i++) {
        ...
        if (!compare(filteredInstance, filteredInstanceSource)) {
          System.out.println((i + 1)
              + ". instance (Filter/Source code): "
              + filteredInstance
              + " != "
              + filteredInstanceSource);
        }
      }
    }
  ...
  @Override public String[] getOptions() {
    ...
    if (getFilter() != null) {
      result.add("-W");
      result.add(
          getFilter().getClass().getName() + " " + Utils.joinOptions(getFilter().getOptions()));
    }
    ...
    if (result) {
      for (i = 0; i < inst1.numAttributes(); i++) {
        ...
        if (inst1.value(i) != inst2.value(i)) {
          result = false;
          System.out.println("Values at position "
              + (i + 1)
              + " differ (Filter/Source code): "
              + inst1.value(i)
              + " != "
              + inst2.value(i));
          break;
