 ...
 */
public class NominalToBinary extends Filter
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tSets if binary attributes are to be coded as nominal ones.", "N", 0, "-N"));
  ...
  private void setOutputFormatNominal() {
    ...
    for (int i = 0; i < getInputFormat().numAttributes(); i++) {
      ...
      if (att.isNominal() && i != getInputFormat().classIndex() && (att.numValues() > 2
          || m_TransformAll
          || m_Numeric)) {
