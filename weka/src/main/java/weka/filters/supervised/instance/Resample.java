...
public class Resample extends Filter implements SupervisedFilter, OptionHandler {
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    result.addElement(
        new Option("\tDisables replacement of instances\n" + "\t(default: with replacement)",
            "no-replacement", 0, "-no-replacement"));

    result.addElement(
        new Option("\tInverts the selection - only available with '-no-replacement'.", "V", 0,
            "-V"));
  ...
  protected void createSubsample() {
    ...
    for (int i = 0; i < data.numClasses(); i++) {
      ...
      int sampleSize = (int) ((m_SampleSizePercent / 100.0) * ((1 - m_BiasToUniformClass)
      ...
      numInstancesToSample[i] = sampleSize;
    }
