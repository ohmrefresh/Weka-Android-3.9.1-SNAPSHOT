...
/**
 * <!-- globalinfo-start -->
 * Reweights the instances in the data so that each class has the same total weight. The total sum
 * of weights accross all instances will be maintained. Only the weights in the first batch of data
 * received by this filter are changed, so it can be used with the FilteredClassifier.
 * <p/>
 * <!-- globalinfo-end -->
 *
 * <!-- options-start -->
 ...
 */
public class ClassBalancer extends SimpleBatchFilter
    implements SupervisedFilter, WeightedInstancesHandler {
  ...
  @Override protected Instances process(Instances instances) throws Exception {
    ...
    for (int i = 0; i < instances.numInstances(); i++) {
      ...
      newInst.setWeight(
          factor * newInst.weight() / sumOfWeightsPerClass[(int) newInst.classValue()]);
    }
    return result;
  }
