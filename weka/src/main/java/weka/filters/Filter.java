...
public abstract class Filter
   ...
   */
  public static String wekaStaticWrapper(Sourcable filter, String className, Instances input,
      Instances output) throws Exception {
    ...
    for (i = 0; i < output.numAttributes(); i++) {
      ...
      if (output.attribute(i).isNumeric()) {
        ...
        result.append(
            "    atts.add(new Attribute(\"" + output.attribute(i).name() + "\", attValues));\n");
      } else {
  ...
  public static void runFilter(Filter filter, String[] options) {
    try {
      ...
      if ((e.toString().indexOf("Help requested") == -1) && (e.toString().indexOf("Filter options")
          == -1)) {
        e.printStackTrace();
      } else {
  ...
  protected void copyValues(Instance instance, boolean isInput) {
    ...
    RelationalLocator.copyRelationalValues(instance, (isInput) ? m_InputFormat : m_OutputFormat,
        (isInput) ? m_InputRelAtts : m_OutputRelAtts);

    StringLocator.copyStringValues(instance, (isInput) ? m_InputFormat : m_OutputFormat,
   ...
   */
  protected void copyValues(Instance instance, boolean instSrcCompat, Instances srcDataset,
    ...
    StringLocator.copyStringValues(instance, instSrcCompat, srcDataset, m_InputStringAtts,
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option("\tIf set, filter is run in debug mode and\n"
        + "\tmay output additional info to the console", "output-debug-info", 0,
        "-output-debug-info"));
