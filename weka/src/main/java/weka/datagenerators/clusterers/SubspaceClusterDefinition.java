...
public class SubspaceClusterDefinition extends ClusterDefinition {
   ...
   */
  @Override protected void setDefaults() throws Exception {
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    result.addElement(
        new Option("\tGenerates randomly distributed instances in the cluster.", "A", 1,
            "-A <range>"));

    result.addElement(
        new Option("\tGenerates uniformly distributed instances in the cluster.", "U", 1,
            "-U <range>"));

    result.addElement(
        new Option("\tGenerates gaussian distributed instances in the cluster.", "G", 1,
            "-G <range>"));
    ...
    result.addElement(new Option("\tThe range of number of instances per cluster (default "
        + defaultMinInstNum()
        + ".."
        + defaultMaxInstNum()
        + ").", "N", 1, "-N <num>..<num>"));
    ...
    result.addElement(
        new Option("\tUses integer instead of continuous values (default continuous).", "I", 0,
            "-I"));
  ...
  @Override public void setOptions(String[] options) throws Exception {
    ...
    if (tmpStr.length() != 0) {
      fromToStr = tmpStr;
      setClusterType(
          new SelectedTag(SubspaceCluster.UNIFORM_RANDOM, SubspaceCluster.TAGS_CLUSTERTYPE));
      typeCount++;
    }
    ...
    if (tmpStr.length() != 0) {
      fromToStr = tmpStr;
      setClusterType(
          new SelectedTag(SubspaceCluster.TOTAL_UNIFORM, SubspaceCluster.TAGS_CLUSTERTYPE));
      typeCount++;
    }
    ...
    if (Utils.getFlag('I', options)) {
      setClusterSubType(
          new SelectedTag(SubspaceCluster.INTEGER, SubspaceCluster.TAGS_CLUSTERSUBTYPE));
    } else {
      setClusterSubType(
          new SelectedTag(SubspaceCluster.CONTINUOUS, SubspaceCluster.TAGS_CLUSTERSUBTYPE));
    }
  }
    ...
    for (int i = 0; i < m_numClusterAttributes; i++) {
      ...
      if (getParent() != null) {
        ...
        if (parent.isBoolean(m_attrIndices[i])) {
          ...
          if (((m_valueA[i] != 0.0) && (m_valueA[i] != 1.0)) || ((m_valueB[i] != 0.0)
              && (m_valueB[i] != 1.0))) {
