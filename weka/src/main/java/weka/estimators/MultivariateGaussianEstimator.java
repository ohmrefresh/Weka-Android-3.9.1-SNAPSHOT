...
public class MultivariateGaussianEstimator implements MultivariateEstimator, Serializable {
   ...
   * of the per-group covariance matrices. The pooled mean vector is the mean vector for all
   * observations.
   *
  ...
  @Override public void estimate(double[][] observations, double[] weights) {
    ...
    // Compute inverse of covariance matrix
    DenseCholesky chol =
        new DenseCholesky(observations[0].length, true).factor((UpperSPDDenseMatrix) cov);
    covarianceInverse =
        new UpperSPDDenseMatrix(chol.solve(Matrices.identity(observations[0].length)));
  ...
  @Override public double logDensity(double[] valuePassed) {
    ...
    return lnconstant - 0.5 * x.dot(
        covarianceInverse.mult(x.add(-1.0, mean), new DenseVector(x.size())));
  }
  ...
  private DenseVector weightedMean(double[][] matrix, DenseVector weights) {
    ...
    return (DenseVector) new DenseMatrix(matrix).transMult(weights,
        new DenseVector(matrix[0].length));
  }

  /**
   * Computes the estimate of the covariance matrix.
   *
   * @param matrix A multi-dimensional array containing the matrix values (assumed to contain at
   * least one row).
   * @param weights The observation weights, normalized to sum to 1.
   * @param mean The values' mean vector.
   * @return The covariance matrix, including the ridge.
   */
  private UpperSPDDenseMatrix weightedCovariance(double[][] matrix, DenseVector weights,
      Vector mean) {
    ...
    UpperSPDDenseMatrix covT =
        (UpperSPDDenseMatrix) new UpperSPDDenseMatrix(cols).rank1(transposed);
