...
public class NormalEstimator extends Estimator
   ...
   */
  @Override public void addValue(double data, double weight) {
   ...
   */
  @Override public double getProbability(double data) {
  ...
  protected void computeParameters() {
    if (m_SumOfWeights > 0) {
      ...
      double stdDev =
          Math.sqrt(Math.abs(m_SumOfValuesSq - m_Mean * m_SumOfValues) / m_SumOfWeights);
  ...
  @Override public String toString() {
    ...
    return "Normal Distribution. Mean = "
        + Utils.doubleToString(m_Mean, 4)
        + " StandardDev = "
        + Utils.doubleToString(m_StandardDev, 4)
        + " WeightSum = "
        + Utils.doubleToString(m_SumOfWeights, 4)
        + " Precision = "
        + m_Precision
        + "\n";
  }

  @Override public NormalEstimator aggregate(NormalEstimator toAggregate) throws Exception {
