 ...
 * and applies the leave-one-out bootstrap for model selection. Can alternatively use normalized
 * entropy.
 *
...
public class UnivariateMixtureEstimator
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    options.addElement(
        new Option("\tMaximum number of components to use (default: 5).", "M", 1, "-M"));
    options.addElement(
        new Option("\tSeed for the random number generator (default: 1).", "S", 1, "-S"));
    options.addElement(
        new Option("\tThe number of bootstrap runs to use (default: 10).", "B", 1, "-B"));
  ...
  protected double entropy(MM mixtureModel) {
    ...
    double entropy = 0;
    for (int j = 0; j < m_NumValues; j++) {
      entropy += m_Weights[j] * ContingencyTables.entropy(
          Utils.logs2probs(mixtureModel.logJointDensities(m_Values[j])));
    }
