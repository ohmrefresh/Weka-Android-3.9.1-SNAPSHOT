...
public class UnivariateEqualFrequencyHistogramEstimator
  ...
  public static void main(String[] args) {
    ...
    for (int j = 1; j < 5; j++) {
      ...
      UnivariateEqualFrequencyHistogramEstimator eHistogram =
      ...
      System.out.println(
          "Coverage at 0.95 level for histogram intervals: " + histogramCovered / 1000);
      ...
      System.out.println(
          "Coverage at 0.8 level for histogram intervals: " + histogramCovered / 1000);
  ...
  @Override public double logDensity(double value) {
    ...
    if ((index == -1) || (index == -m_Boundaries.length - 1)) {
      ...
      return (CONST - Math.log(m_Width) - 0.5 * (val * val / (m_Width * m_Width))) - Math.log(
          m_SumOfWeights + 2);
    }
    ...
    double densSmearedOut =
        1.0 / ((m_SumOfWeights + 2) * (m_Boundaries[m_Boundaries.length - 1] - m_Boundaries[0]));
  ...
  @Override public String toString() {
    ...
    if (m_Boundaries != null) {
      ...
      for (int i = 0; i < m_Boundaries.length - 1; i++) {
        text.append(
            m_Boundaries[i] + "\t" + m_Boundaries[i + 1] + "\t" + m_Weights[i] + "\t" + Math.exp(
                logDensity((m_Boundaries[i + 1] + m_Boundaries[i]) / 2)) + "\n");
      }
    }
