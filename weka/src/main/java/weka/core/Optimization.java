...
public abstract class Optimization implements TechnicalInformationHandler, RevisionHandler {
  ...
  public double[] findArgmin(double[] initX, double[][] constraints) throws Exception {
    ...
    double sum = 0;
    double[] grad = evaluateGradient(initX), oldGrad, oldX, deltaGrad = new double[l], deltaX =
        new double[l], direct = new double[l], x = new double[l];
    ...
    for (int step = 0; step < m_MAXITS; step++) {
      ...
      if (m_IsZeroStep) { // Zero step, simply delete rows/cols of D and L
        ...
        for (int g = 0; g < l; g++) {
          ...
          double tmp =
              Math.abs(grad[g]) * Math.max(Math.abs(direct[g]), 1.0) / Math.max(Math.abs(m_f), 1.0);
          if (tmp > test) {
            test = tmp;
          }
        }
        ...
        if (finish) {
          ...
          for (int m = size - 1; m >= 0; m--) {
            ...
            if (x[index] >= constraints[1][index]) {
              ...
              throw new Exception(
                  "x[" + index + "] not fixed on the" + " bounds where it should have been!");
            }
          ...
          for (int mmm = 0; mmm < toFree.size(); mmm++) {
            ...
            if (x[freeIndx] <= constraints[0][freeIndx]) {// Lower bound
              ...
              if (m_Debug) {
                System.err.println(
                    "Free variable " + freeIndx + " from bound " + nwsBounds[0][freeIndx]);
              }
            } else { // Upper bound
              nwsBounds[1][freeIndx] = constraints[1][freeIndx];
              if (m_Debug) {
                System.err.println(
                    "Free variable " + freeIndx + " from bound " + nwsBounds[1][freeIndx]);
              }
            }
      ...
      for (int m = 0; m < LDIR.length; m++) {
        if (Double.isNaN(LDIR[m])) {
          throw new Exception(
              "L*direct[" + m + "] is NaN!" + "|-g=" + b[m] + "|" + isFixed[m] + "|diag=" + D[m]);
        }
      }
   ...
   * @throws Exception if an error occurs
   */
  public double[] lnsrch(double[] xold, double[] gradient, double[] direct, double stpmax,
      boolean[] isFixed, double[][] nwsBounds, DynamicIntArray wsBdsIndx) throws Exception {
    ...
    if (m_Debug) {
      System.err.println(
          "fold:  " + Utils.doubleToString(fold, 10, 7) + "\n" + "sum:  " + Utils.doubleToString(
    ...
    for (k = 0; ; k++) {
      ...
      if (m_Debug) {
        ...
        System.err.println(
            "threshold: " + Utils.doubleToString(fold + m_ALF * alam * m_Slope, 10, 7));
      }
    ...
    if (m_Debug) {
      System.err.println(
          "Last stage of searching for beta condition (alam between "
              + Utils.doubleToString(lo, 10, 7)
              + " and "
    ...
    if (newSlope < m_BETA * m_Slope) { // Cannot satisfy beta condition, take lo
      ...
      System.err.println(
          "Both alpha and beta conditions are satisfied. alam=" + Utils.doubleToString(alam, 10,
              7));
    }
   ...
   */
  protected void updateCholeskyFactor(Matrix L, double[] D, double[] v, double coeff,
    ...
    if (coeff > 0.0) {
      ...
      for (int j = 0; j < n; j++) {
        ...
        if ((j < n - 1) && (Double.isNaN(sigma) || Double.isInfinite(sigma))) {
          throw new Exception("sigma NaN/Inf! rho="
              + rho
              + ",theta="
              + theta
              + ",P["
