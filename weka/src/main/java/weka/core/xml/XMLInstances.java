...
public class XMLInstances extends XMLDocument implements Serializable {
    ...
    if ((att.getMetadata() != null) && (att.getMetadata().size() > 0)) {
      ...
      while (enm.hasMoreElements()) {
        ...
        property.appendChild(
            m_Document.createTextNode(validContent(att.getMetadata().getProperty(tmpStr, ""))));
      }
    }
   ...
   * @throws Exception if generation fails, e.g., due to unknown attribute type
   */
  protected ArrayList<Attribute> createAttributes(Element parent, int[] classIndex)
      throws Exception {
    Vector<Element> list;
    ...
    for (i = 0; i < list.size(); i++) {
      ...
      if (node.getAttribute(ATT_MISSING).equals(VAL_YES)) {
        ...
        switch (header.attribute(index).type()) {
          ...
          default:
            throw new Exception(
                "Attribute type " + header.attribute(index).type() + " is not supported!");
        }
      }
    }
