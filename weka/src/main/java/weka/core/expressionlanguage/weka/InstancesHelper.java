...
public class InstancesHelper implements VariableDeclarations, Macro, MacroDeclarations {
  ...
  public Node evaluate(Node... params) throws SemanticException {
    ...
    if (params[0] instanceof Value) return new isMissing((Value) params[0]);
  ...
  @Override public boolean hasVariable(String name) {
    ...
    return 0 <= index && index < dataset.numAttributes();
  }
  ...
  private int getIndex(String attribute) {
    ...
    if (CLASS.equals(attribute)) return dataset.classIndex();
  ...
  @Override public Node getVariable(String name) {
    ...
    if (dataset.attribute(index).isNumeric()) return new DoubleValue(index);
