...
public class IfElseMacro implements MacroDeclarations, Macro {
  ...
  @Override public Macro getMacro(String name) {
    if (IF_ELSE.equals(name)) return this;
  ...
  @Override public Node evaluate(Node... params) throws SemanticException {
    if (params.length != 3) throw new SemanticException("ifelse takes exactly 3 arguments!");
    ...
    if (params[1] instanceof BooleanExpression && params[2] instanceof BooleanExpression) {
      ...
      return new BooleanIfElse((BooleanExpression) params[0], (BooleanExpression) params[1],
          (BooleanExpression) params[2]);
    } else if (params[1] instanceof DoubleExpression && params[2] instanceof DoubleExpression) {

      return new DoubleIfElse((BooleanExpression) params[0], (DoubleExpression) params[1],
          (DoubleExpression) params[2]);
    } else if (params[1] instanceof StringExpression && params[2] instanceof StringExpression) {

      return new StringIfElse((BooleanExpression) params[0], (StringExpression) params[1],
          (StringExpression) params[2]);
    }

    throw new SemanticException(
        "ifelse's second and third parameter must be doubles, booleans or Strings!");
  }
  ...
  private static class DoubleIfElse implements DoubleExpression {
    ...
    public DoubleIfElse(BooleanExpression condition, DoubleExpression ifPart,
    ...
    @Override public double evaluate() {
      if (condition.evaluate()) return ifPart.evaluate();
  ...
  private static class BooleanIfElse implements BooleanExpression {
    ...
    public BooleanIfElse(BooleanExpression condition, BooleanExpression ifPart,
  ...
  private static class StringIfElse implements StringExpression {
    ...
    public StringIfElse(BooleanExpression condition, StringExpression ifPart,
