...
public class JavaMacro implements MacroDeclarations, Macro {
   ...
   */
  @Override public Node evaluate(Node... nodes) throws SemanticException {
    if (nodes.length < 2) {
    ...
    if (!(nodes[0] instanceof StringConstant && nodes[1] instanceof StringConstant)) {
      throw new SemanticException(
          JAVA_MACRO + "'s first and second argument must be String constants!");
    }
    ...
    try {
      ...
      throw new SemanticException(
          "Failed to load method '" + className + "." + name + "' in " + JAVA_MACRO + " macro!", e);
    }
    ...
    for (int i = 0; i < parameterTypes.size() && i < parameterNodes.length; i++) {
      if (parameterTypes.get(i).equals(Boolean.TYPE)
          && parameterNodes[i] instanceof BooleanExpression) {
        continue;
      }
      if (parameterTypes.get(i).equals(Double.TYPE)
          && parameterNodes[i] instanceof DoubleExpression) {
        continue;
      }
      if (parameterTypes.get(i).equals(String.class)
          && parameterNodes[i] instanceof StringExpression) {
        continue;
      }
      throw new SemanticException("Type error in " + JAVA_MACRO + " macro!");
    }

    if (returnType.equals(Boolean.TYPE)) return new BooleanJavaMethod(m, parameterNodes);
    if (returnType.equals(Double.TYPE)) return new DoubleJavaMethod(m, parameterNodes);
    if (returnType.equals(String.class)) return new StringJavaMethod(m, parameterNodes);
  ...
  private String parseSignature(String signature, List<Class<?>> types) throws InvalidSignature {
    ...
    if (tokens.size() < 4) throw new InvalidSignature("Not enough tokens in '" + signature + "'");
    ...
    for (; i < tokens.size() && !")".equals(tokens.get(i)); i++) {
      ...
      if (!first && !",".equals(tokens.get(i))) {
        throw new InvalidSignature(
            "Missing comma between parameters, got '" + tokens.get(i) + "' instead");
      }

      if (!first) ++i;

      // get parameter
      if (i >= tokens.size()) throw new InvalidSignature("No parameter after comma!");
  ...
  private static class BooleanJavaMethod extends JavaMethod implements BooleanExpression {
    ...
    @Override public boolean evaluate() {
      try {
        ...
        throw new RuntimeException("Failed to execute java function '" + method.getName() + "'!",
            e);
      }
    }
  }
