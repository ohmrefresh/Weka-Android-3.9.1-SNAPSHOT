 ...
 * @see Loader
 */
public class TextDirectoryLoader extends AbstractLoader
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    result.add(
        new Option("\tStores the filename in an additional attribute.\n" + "\t(default: off)", "F",
            0, "-F"));

    result.add(
        new Option("\tThe directory to work on.\n" + "\t(default: current directory)", "dir", 0,
            "-dir <directory>"));
  ...
  @Override public Instance getNextInstance(Instances structure) throws IOException {
    ...
    if (found) {
      ...
      File txt = new File(directoryPath
          + File.separator
  ...
  @Override public void run(Object toRun, String[] args) throws IllegalArgumentException {
    ...
    if (args.length > 0) {
      ...
      Enumeration<Option> enm = new TextDirectoryLoader().listOptions();
      while (enm.hasMoreElements()) {
