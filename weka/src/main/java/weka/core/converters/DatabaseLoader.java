...
public class DatabaseLoader extends AbstractLoader
  ...
  @OptionMetadata(displayName = "Query", description = "The query to execute",
      displayOrder = 4) public String getQuery() {
    return m_query;
  }
      ...
      if (m_structure == null) {
        if (m_checkForTable) {
          if (!m_DataBaseConnection.tableExists(endOfQuery(true))) {
            throw new IOException("Table does not exist according to metadata from JDBC driver. "
  ...
  @OptionMetadata(
      ...
      displayOrder = 5) public String getKeys() {
  ...
  @OptionMetadata(
      ...
      displayOrder = 8) public File getCustomPropsFile() {
  ...
  @OptionMetadata(displayName = "Create sparse instances", description = "Return sparse "
      + "rather than normal instances", displayOrder = 6) public boolean getSparseData() {
  ...
  @OptionMetadata(displayName = "Password",
      description = "The database password", displayOrder = 3) public String getPassword() {
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.add(new Option("\tList of column names uniquely defining a DB row\n"
        ...
        + "\tIf not specified, the key will be determined automatically,\n"
        + "\tif possible with the used JDBC driver.\n"
        + "\tThe auto ID column created by the DatabaseSaver won't be loaded.", "P", 1,
        "-P <list of column names>"));
