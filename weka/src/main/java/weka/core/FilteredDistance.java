...
public class FilteredDistance implements DistanceFunction, OptionHandler, Serializable {
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    result.add(
        new Option("\tThe filter to use. (default: weka.unsupervised.attribute.RandomProjection",
            "F", 1, "-F"));

    result.addElement(
        new Option("\tThe distance function to use. (default: weka.core.EuclideanDistance", "E", 0,
            "-E"));
    ...
    if (m_Distance instanceof OptionHandler) {
      ...
      result.addAll(Collections.list(m_Distance.listOptions()));
    }
  ...
  @Override public void setOptions(String[] options) throws Exception {
    ...
    if (distance.length() != 0) {
      ...
      setDistance(
          (DistanceFunction) Utils.forName(DistanceFunction.class, className, distanceSpec));
    } else {
