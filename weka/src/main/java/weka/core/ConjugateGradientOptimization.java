...
public abstract class ConjugateGradientOptimization extends Optimization
  ...
  @Override public double[] findArgmin(double[] initX, double[][] constraints) throws Exception {
    ...
    double[] grad = evaluateGradient(initX), oldGrad, oldX, deltaX = new double[l], direct =
        new double[l], x = new double[l];
    ...
    for (int step = 0; step < m_MAXITS; step++) {
      ...
      for (int g = 0; g < l; g++) {
        double tmp =
            Math.abs(grad[g]) * Math.max(Math.abs(directB[g]), 1.0) / Math.max(Math.abs(m_f), 1.0);
        if (tmp > test) {
          test = tmp;
        }
      }
