...
public abstract class Logger implements RevisionHandler {
  ...
  public static void log(Level level, String msg) {
    ...
    synchronized (logger) {
      ...
      if (logger.getMinLevel() == Level.ALL) {
        log = true;
      } else if (level.getOrder() >= logger.getMinLevel().getOrder()) log = true;
      if (!log) return;
  ...
  public static Logger getSingleton() {
    ...
    if (m_Singleton == null) {
      // logger
      classname =
          m_Properties.getProperty("weka.core.logging.Logger", ConsoleLogger.class.getName());
      try {
      ...
      m_DateFormat = new SimpleDateFormat(
          m_Properties.getProperty("weka.core.logging.DateFormat", "yyyy-MM-dd HH:mm:ss"));
    }
