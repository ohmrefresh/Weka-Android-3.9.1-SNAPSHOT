...
public class Jython implements Serializable, RevisionHandler {
  ...
  public final static String CLASS_PYTHONOBJECTINPUTSTREAM =
  ...
  public static Object newInstance(File file, Class<?> template, File[] paths) {
    ...
    if (paths.length > 0) {
      ...
      for (i = 0; i < paths.length; i++) {
        if (!currentPaths.contains(paths[i].getAbsolutePath())) {
          invoke(interpreter, "exec", new Class[] { String.class }, new Object[] {
              "sys.path.append('" + paths[i].getAbsolutePath() + "')"
          });
        }
      }
    }
    ...
    try {
      result = invoke(interpreter, "get", new Class[] {
          String.class, Class.class
      }, new Object[] { instanceName, template });
    } catch (Exception ex) {
      ex.printStackTrace();
    }
   ...
   */
  public static Object invoke(Object o, String methodName, Class<?>[] paramClasses,
      Object[] paramValues) {
    Method m;
    Object result;
