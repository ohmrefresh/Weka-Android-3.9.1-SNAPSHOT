...
public class Attribute implements Copyable, Serializable, RevisionHandler {
   ...
   * @param createStringAttribute if true, a string attribute will be created, otherwise a numeric
   * one.
   */
  ...
  public Attribute(String attributeName, boolean createStringAttribute) {
    ...
    this(attributeName, createStringAttribute, null);
  }

  /**
   * Constructor for a numeric or string attribute, where metadata is supplied. Provides an
   * alternative
   * way for creating string attributes.
   *
   * @param attributeName the name for the attribute
  ...
  public Attribute(String attributeName, String dateFormat) {
    ...
    this(attributeName, dateFormat, null);
  }
  ...
  public Attribute(String attributeName, Instances header) {
    ...
    this(attributeName, header, null);
  }
  ...
  public static void main(String[] ops) {
    ...
    try {
      ...
      System.out.println(
          "Index of value \"first\" of \"position\" (should be 0): " + position.indexOfValue(
              "first"));
  ...
  @Override public/* @ pure non_null @ */Object copy() {

    return copy(m_Name);
  }

  /**
   * Tests if given attribute is equal to this attribute. Attribute indices are ignored in the
   * comparison.
   *
   * @param other the Object to be compared to this attribute
   * @return true if the given attribute is equal to this attribute
   */
  @Override public final/* @ pure @ */boolean equals(Object other) {
   ...
   */
  @Override public final/* @ pure @ */int hashCode() {
  ...
  @Override public final String toString() {
    ...
    switch (m_Type) {
      ...
      case RELATIONAL:
        ...
        Enumeration<Attribute> enm =
            ((RelationalAttributeInfo) m_AttributeInfo).m_Header.enumerateAttributes();
  ...
  public final int indexOfValue(String value) {
    ...
    if (value.length() > STRING_COMPRESS_THRESHOLD) {
      try {
        ...
        System.err.println(
            "Couldn't compress string attribute value -" + " searching uncompressed.");
      }
    }
  ...
  public final String equalsMsg(Object other) {
    ...
    if (isNominal() && att.isNominal()) {
      if (((NominalAttributeInfo) m_AttributeInfo).m_Values.size()
        ...
        return "Different number of labels: "
            + ((NominalAttributeInfo) m_AttributeInfo).m_Values.size()
            + " != "
            + ((NominalAttributeInfo) att.m_AttributeInfo).m_Values.size();
      }
      ...
      for (int i = 0; i < ((NominalAttributeInfo) m_AttributeInfo).m_Values.size(); i++) {
        if (!((NominalAttributeInfo) m_AttributeInfo).m_Values.get(i).
          ...
          return "Labels differ at position "
              + (i + 1)
              + ": "
      ...
      case STRING:
        ...
        if (string.length() > STRING_COMPRESS_THRESHOLD) {
          try {
            ...
            System.err.println(
                "Couldn't compress string attribute value -" + " storing uncompressed.");
          }
        }
        ((NominalAttributeInfo) m_AttributeInfo).m_Hashtable.
  ...
  final void setValue(int index, Instances data) {
    ...
    if (isRelationValued()) {
      if (!data.equalHeaders(((RelationalAttributeInfo) m_AttributeInfo).m_Header)) {
        throw new IllegalArgumentException(
            "Can't set relational value. " + "Headers not compatible.\n" + data.equalHeadersMsg(
                ((RelationalAttributeInfo) m_AttributeInfo).m_Header));
      }
      ((NominalAttributeInfo) m_AttributeInfo).m_Values =
          Utils.cast(((NominalAttributeInfo) m_AttributeInfo).m_Values.clone());
