...
public class TopDownConstructor extends BallTreeConstructor implements TechnicalInformationHandler {
  ...
  @Override public BallNode buildTree() throws Exception {
    ...
    root.setRadius(
        BallNode.calcRadius(m_InstList, m_Instances, root.getPivot(), m_DistanceFunction));
  ...
  @Override public int[] addInstance(BallNode node, Instance inst) throws Exception {
    ...
    if (node.m_Left != null && node.m_Right != null) { // if node is not a leaf
      ...
      rightDist =
          m_DistanceFunction.distance(inst, node.m_Right.getPivot(), Double.POSITIVE_INFINITY);
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tBall splitting algorithm to use.", "S", 1, "-S <classname and options>"));
  ...
  protected void splitNodes(BallNode node, int depth, final double rootRadius) throws Exception {
    ...
    if (node.m_NumInstances <= m_MaxInstancesInLeaf || (rootRadius == 0 ? true
    ...
    if (m_FullyContainChildBalls) {
      double radius =
          BallNode.calcRadius(node.m_Left, node.m_Right, node.getPivot(), m_DistanceFunction);
  ...
  @Override public void setOptions(String[] options) throws Exception {
    ...
    if (optionString.length() != 0) {
      ...
      setBallSplitter(
          (BallSplitter) Utils.forName(BallSplitter.class, className, nnSearchClassSpec));
    } else {
