...
/**
 * <!-- globalinfo-start -->
 * The class that splits a node into two based on the midpoint value of the dimension in which the
 * node's rectangle is widest. If after splitting one side is empty then it is slided towards the
 * non-empty side until there is at least one point on the empty side.<br/>
 * <br/>
...
public class SlidingMidPointOfWidestSide extends KDTreeNodeSplitter
  ...
  public String globalInfo() {
    return "The class that splits a node into two based on the midpoint value of "
  ...
  public void splitNode(KDTreeNode node, int numNodesCreated, double[][] nodeRanges,
    ...
    node.m_Left = new KDTreeNode(numNodesCreated + 1, node.m_Start, rightStart - 1,
        m_EuclideanDistance.initializeRanges(m_InstList, node.m_Start, rightStart - 1), widths);
