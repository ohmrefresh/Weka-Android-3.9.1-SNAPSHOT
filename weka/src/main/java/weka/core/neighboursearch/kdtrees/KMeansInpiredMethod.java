...
public class KMeansInpiredMethod extends KDTreeNodeSplitter implements TechnicalInformationHandler {
   ...
   */
  @Override public void splitNode(KDTreeNode node, int numNodesCreated, double[][] nodeRanges,
    ...
    double leftAttSum[] = new double[m_Instances.numAttributes()], rightAttSum[] =
        new double[m_Instances.numAttributes()], leftAttSqSum[] =
        new double[m_Instances.numAttributes()], rightAttSqSum[] =
        new double[m_Instances.numAttributes()], rightSqMean, leftSqMean, leftSqSum, rightSqSum,
    ...
    for (int dim = 0; dim < m_Instances.numAttributes(); dim++) {
      ...
      for (int i = node.m_Start; i <= node.m_End - 1; i++) {
        ...
        if (minSum > (leftSqSum + rightSqSum)) {
          ...
          if (i < node.m_End) {
            splitVal = (m_Instances.instance(m_InstList[i]).value(dim) + m_Instances.instance(
                m_InstList[i + 1]).value(dim)) / 2;
          } else {
    ...
    if (rightStart == node.m_Start || rightStart > node.m_End) {
      System.out.println("node.m_Start: "
          + node.m_Start
          + " node.m_End: "
          + node.m_End
          + " splitDim: "
          ...
          + splitVal
          + " node.min: "
          + node.m_NodeRanges[splitDim][MIN]
          + " node.max: "
          + node.m_NodeRanges[splitDim][MAX]
          + " node.numInstances: "
          + node.numInstances());
      ...
      if (rightStart == node.m_Start) {
        throw new Exception("Left child is empty in node "
            + node.m_NodeNumber
            + ". Not possible with "
            + "KMeanInspiredMethod splitting method. Please "
            + "check code.");
      } else {
        throw new Exception("Right child is empty in node "
            + node.m_NodeNumber
            + ". Not possible with "
            + "KMeansInspiredMethod splitting method. Please "
            + "check code.");
      }
    }
  ...
  protected int rearrangePoints(int[] indices, final int startidx, final int endidx,
    ...
    for (int i = startidx; i <= endidx; i++) {
      if (m_EuclideanDistance.valueIsSmallerEqual(m_Instances.instance(indices[i]), splitDim,
          splitVal)) {
        left++;
        tmp = indices[left];
