  ...
  @Override public double getMeasure(String additionalMeasureName) {
    if (m_Stats == null) {
      throw new IllegalArgumentException(
          additionalMeasureName + " not supported (NearestNeighbourSearch)");
    } else {
  ...
  protected class NeighborList implements RevisionHandler {
    ...
    public void insertSorted(double distance, Instance instance) {
      ...
      if (isEmpty()) {
        ...
        if (distance < m_First.m_Distance) {// Insert at head
          ...
          for (; (current.m_Next != null) && (current.m_Next.m_Distance < distance);
              current = current.m_Next) {
          }
    ...
    public void printList() {
      ...
      if (isEmpty()) {
        ...
        while (current != null) {
          System.out.println(
              "Node: instance " + current.m_Instance + ", distance " + current.m_Distance);
          current = current.m_Next;
        }
        System.out.println();
      }
