 ...
 * The connection to dataset is only a reference. For the tree structure the indexes are stored in
 * an array. <br/>
 * Building the tree:<br/>
 * If a node has &lt;maximal-inst-number&gt; (option -L) instances no further splitting is done.
 * Also if the split would leave one side empty, the branch is not split any further even if the
 * instances in the resulting node are more than &lt;maximal-inst-number&gt; instances.<br/>
 * **PLEASE NOTE:** The algorithm can not handle missing values, so it is advisable to run
 * ReplaceMissingValues filter if there are any missing values in the dataset.<br/>
 * <br/>
 * For more information see:<br/>
 * <br/>
 * Jerome H. Friedman, Jon Luis Bentley, Raphael Ari Finkel (1977). An Algorithm for Finding Best
 * Matches in Logarithmic Expected Time. ACM Transactions on Mathematics Software.
 * 3(3):209-226.<br/>
 * <br/>
...
public class KDTree extends NearestNeighbourSearch implements TechnicalInformationHandler {
  ...
  public KDTree() {
    super();
    if (getMeasurePerformance()) m_Stats = m_TreeStats = new TreePerformanceStats();
  }
  ...
  protected void splitNodes(KDTreeNode node, double[][] universe, int depth) throws Exception {
    double[][] nodeRanges =
        m_EuclideanDistance.initializeRanges(m_InstList, node.m_Start, node.m_End);
   ...
   */
  protected void findNearestNeighbours(Instance target, KDTreeNode node, int k, MyHeap heap,
      double distanceToParents) throws Exception {
    if (node.isALeaf()) {
      if (m_TreeStats != null) {
      ...
      for (int idx = node.m_Start; idx <= node.m_End; idx++) {
        ...
        if (heap.size() < k) {
          distance = m_EuclideanDistance.distance(target, m_Instances.instance(m_InstList[idx]),
      ...
      boolean targetInLeft =
          m_EuclideanDistance.valueIsSmallerEqual(target, node.m_SplitDim, node.m_SplitValue);
      ...
      if (heap.size() < k) { // if haven't found the first k
        double distanceToSplitPlane =
            distanceToParents + m_EuclideanDistance.sqDifference(node.m_SplitDim,
  ...
  protected void addInstanceToTree(Instance inst, KDTreeNode node) throws Exception {
    if (node.isALeaf()) {
      ...
      try {
        ...
        System.err.println("m_InstList.length: "
            + m_InstList.length
            + " instList.length: "
            + instList.length
            + "node.m_End+1: "
   ...
   */
  protected void determineAssignments(KDTreeNode node, Instances centers, int[] candidates,
      int[] assignments, double pc) throws Exception {
  ...
  protected double distanceToHrect(KDTreeNode node, Instance x) throws Exception {
    ...
    if (!inside) distance = m_EuclideanDistance.distance(closestPoint, x);
    return distance;
  }
  ...
  protected boolean candidateIsFullOwner(KDTreeNode node, Instance candidate, Instance competitor)
    ...
    boolean isFullOwner =
        m_EuclideanDistance.distance(extremeI, candidate) < m_EuclideanDistance.distance(extremeI,
            competitor);
   ...
   */
  public void assignSubToCenters(KDTreeNode node, Instances centers, int[] centList,
      int[] assignments) throws Exception {
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.add(new Option("\tNode splitting method to use.\n"
        + "\t(default: weka.core.neighboursearch.kdtrees.SlidingMidPointOfWidestSide)", "S", 1,
        "-S <classname and options>"));

    newVector.addElement(
        new Option("\tSet minimal width of a box\n" + "\t(default: 1.0E-2).", "W", 0,
            "-W <value>"));

    newVector.addElement(
        new Option("\tMaximal number of instances in a leaf\n" + "\t(default: 40).", "L", 0, "-L"));
  ...
  public String[] getOptions() {
    ...
    if (getNormalizeNodeWidth()) result.add("-N");
  ...
  public void setMeasurePerformance(boolean measurePerformance) {
    ...
    if (m_MeasurePerformance) {
      if (m_Stats == null) m_Stats = m_TreeStats = new TreePerformanceStats();
    } else {
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (optionString.length() != 0) {
      ...
      setNodeSplitter(
          (KDTreeNodeSplitter) Utils.forName(KDTreeNodeSplitter.class, className, splitMethodSpec));
    } else {
