 ...
 * change the size of the dataset or the order of the instances! Also, the range setting that is
 * specified for the distance function is ignored: all attributes are used for the distance
 * calculation.
 * <p/>
 * <!-- globalinfo-end -->
 *
...
public class FilteredNeighbourSearch extends NearestNeighbourSearch implements CapabilitiesHandler {
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    result.addElement(new Option(
        "\tThe search method to use. (default: weka.core.neighboursearch.LinearNNSearch)", "S", 0,
        "-S"));

    if (m_Filter instanceof OptionHandler) {
      result.addElement(new Option("", "", 0,
          "\nOptions specific to filter " + m_Filter.getClass().getName() + ":"));
      result.addAll(Collections.list(m_Filter.listOptions()));
    }
    ...
    if (m_SearchMethod instanceof OptionHandler) {
      ...
      result.addAll(Collections.list(m_SearchMethod.listOptions()));
    }
  ...
  public void setInstances(Instances data) {
    ...
    try {
      ...
      m_ModifiedSearchMethod =
          (NearestNeighbourSearch) new SerializedObject(getSearchMethod()).getObject();
  ...
  @Override public void setOptions(String[] options) throws Exception {
    ...
    if (searchMethod.length() != 0) {
      ...
      setSearchMethod(
          (NearestNeighbourSearch) Utils.forName(NearestNeighbourSearch.class, className,
              searchMethodSpec));
    } else {
  ...
  @Override public Instance nearestNeighbour(Instance target) throws Exception {
    ...
    return getInstances().instance(
        (int) m_ModifiedSearchMethod.nearestNeighbour(m_AddID.output()).value(m_IndexOfID) - 1);
  }
  ...
  @Override public Instances kNearestNeighbours(Instance target, int k) throws Exception {
    ...
    getFilter().input(target);
    m_AddID.input(getFilter().output());
    Instances neighboursInFilteredSpace =
        m_ModifiedSearchMethod.kNearestNeighbours(m_AddID.output(), k);
