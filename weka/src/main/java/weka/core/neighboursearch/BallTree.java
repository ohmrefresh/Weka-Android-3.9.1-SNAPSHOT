 ...
 * <br/>
 * Jeffrey K. Uhlmann (1991). Satisfying general proximity/similarity queries with metric trees.
 * Information Processing Letters. 40(4):175-179.
 * <p/>
 * <!-- globalinfo-end -->
 *
...
public class BallTree extends NearestNeighbourSearch implements TechnicalInformationHandler {
  ...
  public String globalInfo() {
    return "Class implementing the BallTree/Metric Tree algorithm for "
   ...
   */
  protected void nearestNeighbours(MyHeap heap, BallNode node, Instance target, int k)
      throws Exception {
    ...
    if (distance > -0.000001 && Math.sqrt(heap.peek().distance) < distance - node.getRadius()) {
      ...
      double leftPivotDist = Math.sqrt(
          m_DistanceFunction.distance(target, node.m_Left.getPivot(), Double.POSITIVE_INFINITY));
      double rightPivotDist = Math.sqrt(
          m_DistanceFunction.distance(target, node.m_Right.getPivot(), Double.POSITIVE_INFINITY));
      ...
      for (int i = node.m_Start; i <= node.m_End; i++) {
        ...
        if (heap.totalSize() < k) {
          distance = m_DistanceFunction.distance(target, m_Instances.instance(m_InstList[i]),
              Double.POSITIVE_INFINITY, m_Stats);
          heap.put(m_InstList[i], distance);
        } else {
          MyHeapElement head = heap.peek();
          distance = m_DistanceFunction.distance(target, m_Instances.instance(m_InstList[i]),
              head.distance, m_Stats);
  ...
  public void addInstanceInfo(Instance ins) {
    if (m_Instances != null) m_DistanceFunction.update(ins);
  }
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (optionString.length() != 0) {
      ...
      setBallTreeConstructor(
          (BallTreeConstructor) Utils.forName(BallTreeConstructor.class, className,
              constructorSpec));
    } else {
  ...
  public String[] getOptions() {
    ...
    result.add("-C");
    result.add((m_TreeConstructor.getClass().getName() + " " +
