...
public class BinarySparseInstance extends SparseInstance {
  ...
  @Override public String toString() {
    ...
    for (int i = 0; i < m_Indices.length; i++) {
      ...
      if (m_Dataset == null) {
        ...
        if (m_Dataset.attribute(m_Indices[i]).isNominal() || m_Dataset.attribute(m_Indices[i])
            .isString()) {
    ...
    if (m_Weight != 1.0) {
      text.append(
          ",{" + Utils.doubleToString(m_Weight, AbstractInstance.s_numericAfterDecimalPoint) + "}");
    }
   ...
   * double).
   */
  @Override public final double valueSparse(int indexOfIndex) {
