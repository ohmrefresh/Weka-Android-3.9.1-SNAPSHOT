...
public class NormContinuous extends Expression {
  ...
  public NormContinuous(Element normCont, FieldMetaInfo.Optype opType,
  ...
  private void setUpField() throws Exception {
    ...
    if (m_fieldDefs != null) {
      ...
      if (m_fieldIndex < 0) {
        throw new Exception("[NormContinuous] Can't find field "
            + m_fieldName
      ...
      if (!field.isNumeric()) {
        throw new Exception(
            "[NormContinuous] reference field " + m_fieldName + " must be continuous.");
      }
    }
  }
