...
public class TargetMetaInfo extends FieldMetaInfo implements Serializable {
  ...
  protected TargetMetaInfo(Element target) throws Exception {
    ...
    if (min != null && min.length() > 0) {
      try {
        ...
        throw new Exception(
            "[TargetMetaInfo] can't parse min value for target field " + m_fieldName);
      }
    }
    ...
    if (max != null && max.length() > 0) {
      try {
        ...
        throw new Exception(
            "[TargetMetaInfo] can't parse max value for target field " + m_fieldName);
      }
    }
          ...
          if (valueName != null && valueName.length() > 0) {
            ...
            if (prior != null && prior.length() > 0) {
              try {
                ...
                throw new Exception(
                    "[TargetMetaInfo] Can't parse probability from " + "TargetValue element.");
              }
            } else {
              throw new Exception(
                  "[TargetMetaInfo] No prior probability defined for value " + valueName);
            }
          } else {
            ...
            if (defaultV != null && defaultV.length() > 0) {
              try {
                ...
                throw new Exception(
                    "[TargetMetaInfo] Can't parse default value from " + "TargetValue element.");
              }
            } else {
              throw new Exception(
                  "[TargetMetaInfo] No default value defined for target " + m_fieldName);
            }
          }
        }
      }
  ...
  public double getPriorProbability(String value) throws Exception {
    ...
    if (!found) {
      throw new Exception(
          "[TargetMetaInfo] couldn't find value " + value + "(getPriorProbability)");
    }
    return result;
  }
