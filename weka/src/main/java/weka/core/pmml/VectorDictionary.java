...
public class VectorDictionary implements Serializable {
  ...
  public VectorDictionary(Element vectNode, MiningSchema ms) throws Exception {
    ...
    for (int i = 0; i < fieldRefsL.getLength(); i++) {
      ...
      if (a == null) {
        throw new Exception("[VectorDictionary] can't find field '"
            + fieldName
            + "' in the mining schema/derived fields!");
      }

      FieldMetaInfo.Optype fieldOpt =
          (a.isNumeric()) ? FieldMetaInfo.Optype.CONTINUOUS : FieldMetaInfo.Optype.CATEGORICAL;
    ...
    for (int i = 0; i < vecInstL.getLength(); i++) {
      ...
      if (m_vectorInstances.get(id) != null) {
        throw new Exception(
            "[VectorDictionary] : There is already a vector with ID " + id + " in the dictionary!");
      }
