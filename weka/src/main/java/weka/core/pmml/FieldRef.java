...
public class FieldRef extends Expression {
  ...
  protected void validateField() throws Exception {
    ...
    if (m_fieldDefs != null) {
      ...
      if (a == null) {
        throw new Exception(
            "[FieldRef] Can't find field " + m_fieldName + " in the supplied field definitions");
      }
  ...
  @Override public double getResult(double[] incoming) throws Exception {
        ...
        if (a.isNumeric()) {
          if (m_opType == FieldMetaInfo.Optype.CATEGORICAL
            ...
            throw new IllegalArgumentException(
                "[FieldRef] Optype is categorical/ordinal but matching " + "parameter is not!");
          }
        } else if (a.isNominal()) {
          if (m_opType == FieldMetaInfo.Optype.CONTINUOUS) {
            throw new IllegalArgumentException(
                "[FieldRef] Optype is continuous but matching " + "parameter is not!");
          }
        } else {
    ...
    if (!found) {
      throw new Exception("[FieldRef] this field: "
          + m_fieldName
          + " is not in the supplied "
          + "list of parameters!");
    }
    return result;
  }
