      ...
      if (miningField.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (mfi.getUsageType() == MiningFieldMetaInfo.Usage.ACTIVE
          ...
          if (miningAtt != null) {
            ...
            throw new Exception(
                "Can't find mining field: " + mfi.getName() + " in the data dictionary.");
          }
        }
      }
    }
      ...
      for (int i = 0; i < localDerivedL.getLength(); i++) {
        ...
        if (localDerived.getNodeType() == Node.ELEMENT_NODE) {
          DerivedFieldMetaInfo d =
              new DerivedFieldMetaInfo((Element) localDerived, null /*fieldDefs*/,
  ...
  public void convertNumericAttToNominal(int index, ArrayList<String> newVals) {
    ...
    if (miningSchemaI.attribute(index).isNominal()) {
      throw new IllegalArgumentException(
          "[MiningSchema] convertNumericAttToNominal: attribute is " + "already nominal!");
    }
