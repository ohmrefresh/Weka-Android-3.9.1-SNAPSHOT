...
public class NormDiscrete extends Expression {
  ...
  private void setUpField() throws Exception {
    ...
    if (m_fieldDefs != null) {
      ...
      if (m_fieldIndex < 0) {
        throw new Exception("[NormDiscrete] Can't find field "
            + m_fieldName
      ...
      if (!(m_field.isString() || m_field.isNominal())) {
        throw new Exception(
            "[NormDiscrete] reference field " + m_fieldName + " must be categorical");
      }
      ...
      if (m_field.isNominal()) {
        ...
        if (m_fieldValueIndex < 0) {
          throw new Exception("[NormDiscrete] Unable to find value "
              + m_fieldValue
              + " in nominal attribute "
              + m_field.name());
        }
