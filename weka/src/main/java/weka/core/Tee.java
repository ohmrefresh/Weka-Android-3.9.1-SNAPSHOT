...
public class Tee extends PrintStream implements RevisionHandler {
  ...
  public void clear() {
    ...
    if (getDefault() != null) add(getDefault());
  }
  ...
  public void add(PrintStream p, boolean timestamp, String prefix) {
    if (m_Streams.contains(p)) remove(p);
  ...
  private void printHeader() {
    for (int i = 0; i < size(); i++) {
      // prefix
      if (!m_Prefixes.get(i).equals("")) m_Streams.get(i).print("[" + m_Prefixes.get(i) + "]\t");

      // timestamp
      if (m_Timestamps.get(i).booleanValue()) m_Streams.get(i).print("[" + new Date() + "]\t");
    }
  }
  ...
  public void write(byte buf[], int off, int len) {
    printHeader();
    for (int i = 0; i < size(); i++)
      m_Streams.get(i).write(buf, off, len);
    flush();
  }
