...
public class StringLocator extends AttributeLocator {
  ...
  public static void copyStringValues(Instance instance, boolean instSrcCompat,
    ...
    for (int i = 0; i < srcLoc.getAttributeIndices().length; i++) {
      int instIndex = instSrcCompat ? srcLoc.getActualIndex(srcLoc.getAttributeIndices()[i])
      ...
      Attribute dest =
          destDataset.attribute(destLoc.getActualIndex(destLoc.getAttributeIndices()[i]));
    ...
    for (int i = 0; i < srcIndices.length; i++) {
      ...
      for (int n = 0; n < rel.numInstances(); n++) {
        copyStringValues(rel.instance(n), instSrcCompat, srcDatasetNew, srcStrAttsNew,
            destDatasetNew, destStrAttsNew);
      }
    }
  }
