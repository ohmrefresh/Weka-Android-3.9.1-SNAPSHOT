...
public class Instances extends AbstractList<Instance> implements Serializable, RevisionHandler {
  ...
  @Deprecated public Instances(/* @non_null@ */Reader reader, int capacity) throws IOException {
  ...
  public static void test(String[] argv) {
    ...
    try {
      ...
      if (empty.classAttribute().isNominal()) {
        ...
        copy.renameAttributeValue(copy.classAttribute(), copy.classAttribute().value(0),
            "new_val_name");
  ...
  public static Instances mergeInstances(Instances first, Instances second) {
    ...
    ArrayList<Attribute> newAttributes =
        new ArrayList<Attribute>(first.numAttributes() + second.numAttributes());
    ...
    Instances merged =
        new Instances(first.relationName() + '_' + second.relationName(), newAttributes,
            first.numInstances());
    // Merge each instance
   ...
   * is shallow-copied, so that other Instances objects with a reference to this list are not
   * affected.
   *
  ...
  @Override public void add(int index, /* @non_null@ */Instance instance) {
   ...
   * @param instance the instance to be added
   */
  @Override public boolean add(/* @non_null@ */Instance instance) {
  ...
  // @ requires 0 <= index;
  // @ requires index < numInstances();
  @Override public/* @non_null pure@ */Instance get(int index) {
  ...
  // @ requires 0 <= index;
  // @ requires index < m_Instances.size();
  @Override public Instance set(int index, /* @non_null@ */Instance instance) {
    ...
    for (int i = 0; i < numAttributes(); i++) {
      if (instance.isMissing(i)) {
        ...
        if (!(Utils.eq(instance.value(i), (int) instance.value(i)))) {
          return false;
        } else if (Utils.sm(instance.value(i), 0) || Utils.gr(instance.value(i),
  ...
  public void deleteAttributeAt(int position) {
    ...
    HashMap<String, Integer> newMap =
        new HashMap<String, Integer>((int) ((m_Attributes.size() - 1) / 0.75));
  ...
  public void insertAttributeAt(/* @non_null@ */Attribute att, int position) {
    ...
    if (attribute(att.name()) != null) {
      throw new IllegalArgumentException(
          "Attribute name '" + att.name() + "' already in use at position #" + attribute(att.name())
              .index());
    }
  ...
  public void replaceAttributeAt(/* @non_null@ */Attribute att, int position) {
    ...
    if (!att.name().equals(m_Attributes.get(position).name())) {
      ...
      if ((candidate != null) && (position != candidate.index())) {
        throw new IllegalArgumentException(
            "Attribute name '" + att.name() + "' already in use at position #" +
  ...
  public void renameAttribute(int att, String name) {
    ...
    if (existingAtt != null) {
      if (att == existingAtt.index()) {
        ...
        throw new IllegalArgumentException(
            "Attribute name '" + name + "' already present at position #" + existingAtt.index());
      }
    }
    ...
    if (classAttribute().isNominal()) {
      ...
      while (index < numInstances()) {
        ...
        for (int j = index; j < numInstances(); j++) {
          ...
          if ((instance1.classValue() == instance2.classValue()) || (instance1.classIsMissing()
    ...
    for (int attIndex = 0; attIndex < numAttributes(); attIndex++) {
      if (attribute(attIndex).isNumeric()) {
        if (sumWeights[attIndex] <= 1) {
          ...
          if (vars[attIndex] < 0) vars[attIndex] = 0;
        }
      }
    }
