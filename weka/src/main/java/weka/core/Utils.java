...
public final class Utils implements RevisionHandler {
  ...
  public static Properties readProperties(String resourceName) throws Exception {
    ...
    return new EnvironmentProperties(userProps);
  }
   ...
   * are not acceptable to the object
   */
  @SuppressWarnings("unchecked") protected static Object forNameNoSchemeMatch(Class classType,
  ...
  public static/* @pure@ */int[] stableSort(double[] array) {
    ...
    if (array.length > 1) {
      ...
      while (i < index.length) {
        numEqual = 1;
        for (int j = i + 1; ((j < index.length) && Utils.eq(array[index[i]], array[index[j]]));
            j++) {
          numEqual++;
        }
        if (numEqual > 1) {
   ...
   * @throws Exception if the path cannot be constructed
   */
  ...
  public static void main(String[] ops) {
    ...
    try {
      ...
      // Arrays
      System.out.println(
          "Array-Dimensions of 'new int[][]': " + Utils.getArrayDimensions(new int[][] {}));
      System.out.println(
          "Array-Dimensions of 'new int[][]{{1,2,3},{4,5,6}}': " + Utils.getArrayDimensions(
              new int[][] { { 1, 2, 3 }, { 4, 5, 6 } }));
      String[][][] s = new String[3][4][];
      System.out.println(
          "Array-Dimensions of 'new String[3][4][]': " + Utils.getArrayDimensions(s));
    } catch (Exception e) {
      e.printStackTrace();
    }
  }
  ...
  @Override public String getRevision() {
    return RevisionUtils.extract("$Revision: 12956 $");
  }
}
