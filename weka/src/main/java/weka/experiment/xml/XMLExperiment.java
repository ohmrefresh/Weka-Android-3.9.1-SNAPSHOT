...
public class XMLExperiment extends XMLBasicSerialization {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length > 0) {
      ...
      // read binary and print generated XML
        ...
        // write to XML file
        new XMLExperiment().write(new BufferedOutputStream(new FileOutputStream(args[0] + ".xml")),
            o);
  ...
  @Override protected void writePostProcess(Object o) throws Exception {
    ...
    // classfirst
    node = addElement(m_Document.getDocument().getDocumentElement(), NAME_CLASSFIRST,
        Boolean.class.getName(), false);
    node.appendChild(node.getOwnerDocument()
        .createTextNode(new Boolean(false).toString())); // TODO: get-Method for classFirst in
    // Experiment???
  }
  ...
  public Element writePropertyNode(Element parent, Object o, String name) throws Exception {
    ...
    if ((pnode.value != null) && (pnode.property != null) && (pnode.property.getPropertyType()
        .isPrimitive())) {
