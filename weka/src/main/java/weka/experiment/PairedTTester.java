...
public class PairedTTester implements OptionHandler, Tester, RevisionHandler {
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tSet the index of the column containing the run number", "R", 1,
            "-R <index>"));
    newVector.addElement(
        new Option("\tSet the index of the column containing the fold number", "F", 1,
            "-F <index>"));
    newVector.addElement(new Option("\tSpecify list of columns that specify a unique\n"
        + "\t'result generator' (eg: classifier name and options).\n"
        ...
        "-G <index1,index2-index4,...>"));
    newVector.addElement(
        new Option("\tSet the significance level for comparisons (default 0.05)", "S", 1,
            "-S <significance level>"));
    newVector.addElement(new Option(
        "\tSet the result matrix (classname plus parameters).\n\t(default: weka.experiment.ResultMatrixPlainText)",
        "result-matrix", 1, "-result-matrix <result-matrix-class>"));
    newVector.addElement(new Option("\tShow standard deviations", "V", 0, "-V"));
    newVector.addElement(
        new Option("\tProduce table comparisons in Latex table format", "L", 0, "-L"));
    newVector.addElement(
        new Option("\tProduce table comparisons in CSV table format", "csv", 0, "-csv"));
    newVector.addElement(
        new Option("\tProduce table comparisons in HTML table format", "html", 0, "-html"));
    newVector.addElement(
        new Option("\tProduce table comparisons with only the significance values", "significance",
            0, "-significance"));
    newVector.addElement(
        new Option("\tProduce table comparisons output suitable for GNUPlot", "gnuplot", 0,
            "-gnuplot"));
    newVector.addElement(new Option("", "", 0,
        "\nOptions specific to result matrix " + getResultMatrix().getClass().getName() + ":"));

    newVector.addAll(Collections.list(getResultMatrix().listOptions()));
  ...
  @Override public String[] getOptions() {
    ...
    if (getResultMatrix() instanceof OptionHandler) {
      spec += " " + Utils.joinOptions(getResultMatrix().getOptions());
    }
   ...
   * @see ResultMatrix
   */
  @Override public void setResultMatrix(ResultMatrix matrix) {
  ...
  @Override public void setOptions(String[] options) throws Exception {
    ...
    if (outputOption.length() != 0) {
      ...
      ResultMatrix resultMatrix =
          (ResultMatrix) Utils.forName(Class.forName("weka.experiment.ResultMatrix"),
   ...
   */
  @Override public ResultMatrix getResultMatrix() {
   ...
   */
  @Override public void setShowStdDevs(boolean s) {
    m_ShowStdDevs = s;
  }
   ...
   */
  @Override public boolean getShowStdDevs() {
   ...
   */
  @Override public void assign(Tester tester) {
    ...
    for (int i = 0; i < m_Instances.numInstances(); i++) {
      ...
      for (int m_ResultsetKeyColumn : m_ResultsetKeyColumns) {
        if (current.isMissing(m_ResultsetKeyColumn)) {
          throw new Exception("Instance has missing value in resultset key " + "column " + (
              m_ResultsetKeyColumn
                  + 1) + "!\n" + current);
        }
      }
      for (int m_DatasetKeyColumn : m_DatasetKeyColumns) {
   ...
   */
  @Override public int getNumDatasets() {
   ...
   */
  @Override public int getNumResultsets() {
  ...
  */
  ...
  @Override public String getResultsetName(int index) {
   ...
   */
  @Override public boolean displayResultset(int index) {
    boolean result;
    int i;
   ...
   */
  @Override public PairedStats calculateStatistics(Instance datasetSpecifier, int resultset1Index,
    ...
    if (m_Instances.attribute(comparisonColumn).type() != Attribute.NUMERIC) {
      throw new Exception(
          "Comparison column " + (comparisonColumn + 1) + " (" + m_Instances.attribute(
              comparisonColumn).name() + ") is not numeric");
    }
    if (!m_ResultsetsValid) {
      prepareData();
    }
    ...
    if (dataset1 == null) {
      throw new Exception("No results for dataset="
          + datasetName
          + " for resultset="
      ...
      throw new Exception("Results for dataset="
          + datasetName
   ...
   * @return a value of type 'String'
   */
  @Override public String resultsetKey() {
   ...
   * @return a value of type 'String'
   */
  @Override public String header(int comparisonColumn) {
    ...
    initResultMatrix();
    m_ResultMatrix.addHeader("Tester",
        getClass().getName() + " " + Utils.joinOptions(getOptions()));
   ...
   * @throws Exception if an error occurs
   */
  @Override public int[][] multiResultsetWins(int comparisonColumn, int[][] nonSigWin)
      throws Exception {
        ...
        for (int k = 0; k < getNumDatasets(); k++) {
          try {
            PairedStats pairedStats =
                calculateStatistics(m_DatasetSpecifiers.specifier(k), i, j, comparisonColumn);
   ...
   * @throws Exception if an error occurs
   */
  @Override public String multiResultsetSummary(int comparisonColumn) throws Exception {
   ...
   */
  @Override public String multiResultsetRanking(int comparisonColumn) throws Exception {
   ...
   * @throws Exception if an error occurs
   */
  @Override public String multiResultsetFull(int baseResultset, int comparisonColumn)
      throws Exception {
    ...
    for (int i = 0; i < getNumDatasets(); i++) {
      ...
      for (int j = 0; j < getNumResultsets(); j++) {
        ...
        try {
          PairedStats pairedStats =
              calculateStatistics(m_DatasetSpecifiers.specifier(i), baseResultset, j,
                  comparisonColumn);
          if (!Double.isInfinite(pairedStats.yStats.mean) && !Double.isNaN(
              pairedStats.yStats.mean)) {
          ...
          if (m_ShowStdDevs && !Double.isInfinite(pairedStats.yStats.stdDev) && !Double.isNaN(
              pairedStats.yStats.stdDev)) {
   ...
   */
  @Override public Range getResultsetKeyColumns() {
   ...
   */
  @Override public void setResultsetKeyColumns(Range newResultsetKeyColumns) {
   ...
   */
  @Override public int[] getDisplayedResultsets() {
   ...
   */
  @Override public void setDisplayedResultsets(int[] cols) {
   ...
   */
  @Override public double getSignificanceLevel() {
   ...
   */
  @Override public void setSignificanceLevel(double newSignificanceLevel) {
   ...
   */
  @Override public Range getDatasetKeyColumns() {
   ...
   */
  @Override public void setDatasetKeyColumns(Range newDatasetKeyColumns) {
   ...
   */
  @Override public int getRunColumn() {
   ...
   */
  @Override public void setRunColumn(int newRunColumn) {
   ...
   */
  @Override public int getFoldColumn() {
   ...
   */
  @Override public void setFoldColumn(int newFoldColumn) {
   ...
   */
  @Override public String getSortColumnName() {
    if (getSortColumn() == -1) {
      return "-";
    } else {
   ...
   */
  @Override public int getSortColumn() {
    return m_SortColumn;
  }
   ...
   */
  @Override public void setSortColumn(int newSortColumn) {
    if (newSortColumn >= -1) {
   ...
   */
  @Override public void setInstances(Instances newInstances) {
