...
public class PairedStatsCorrected extends PairedStats {
  ...
  public void calculateDerived() {
    ...
    if (!Double.isNaN(xStats.stdDev) && !Double.isNaN(yStats.stdDev) && !Utils.eq(xStats.stdDev,
        0)) {
      double slope =
          (xySum - xStats.sum * yStats.sum / count) / (xStats.sumSq - xStats.sum * xStats.mean);
    ...
    if (Utils.gr(differencesStats.stdDev, 0)) {
      ...
      double tval = differencesStats.mean / Math.sqrt(
          (1 / count + m_testTrainRatio) * differencesStats.stdDev * differencesStats.stdDev);
