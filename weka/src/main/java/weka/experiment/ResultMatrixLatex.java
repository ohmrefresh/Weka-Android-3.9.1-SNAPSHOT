...
public class ResultMatrixLatex extends ResultMatrix {
  ...
  public String toStringKey() {
    ...
    for (i = 0; i < getColCount(); i++) {
      ...
      result += LEFT_PARENTHESES + (i + 1) + RIGHT_PARENTHESES + " & " + removeFilterName(
          m_ColNames[i]).replace('_', '-').replaceAll("\\\\", "\\\\textbackslash") + " \\\\\n";
    }
    result += "\\end{tabular}\n";
    result += "}\n";
    result += "\\end{table}\n";
  ...
  public String toStringRanking() {
    ...
    for (i = getColCount() - 1; i >= 0; i--) {
      ...
      result +=
          "(" + (curr + 1) + ") & " + Utils.padLeft("" + m_RankingDiff[curr], width) + " & " + Utils
              .padLeft("" + m_RankingWins[curr], width) + " & " + Utils.padLeft(
              "" + m_RankingLosses[curr], width) + "\\\\\n";
    }
