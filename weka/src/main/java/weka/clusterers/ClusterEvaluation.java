...
public class ClusterEvaluation implements Serializable, RevisionHandler {
  ...
  public static String evaluateClusterer(Clusterer clusterer, String[] options) throws Exception {
      ...
      if (trainFileName.length() != 0) {
        ...
        if (classString.length() != 0) {
          ...
          if (train.classIndex() != -1) {
            ...
            System.err.println(
                "Note: using class attribute from dataset, i.e., attribute #" + theClass);
          }
        }
    ...
    if ((clusterer instanceof DensityBasedClusterer) && (doXval == true) && (testFileName.length()
        == 0) && (objectInputFileName.length() == 0)) {
   ...
   */
  public static double crossValidateModel(DensityBasedClusterer clusterer, Instances data,
   ...
   */
  public static String crossValidateModel(String clustererString, Instances data, int numFolds,
  ...
  private static String printClusterStats(Clusterer clusterer, String fileName) throws Exception {
    ...
    if (fileName.length() != 0) {
      ...
      Instances forBatchPredictors = (clusterer instanceof BatchPredictor
          && ((BatchPredictor) clusterer).implementsMoreEfficientBatchPrediction()) ? new Instances(
      ...
      if (forBatchPredictors != null) {
        double[][] dists =
            ((BatchPredictor) clusterer).distributionsForInstances(forBatchPredictors);
        for (double[] d : dists) {
      ...
      for (i = 0; i < cc; i++) {
        if (instanceStats[i] > 0) {
          text.append(Utils.doubleToString(i, clustFieldWidth, 0) + "      " + Utils.doubleToString(
              instanceStats[i], numInstFieldWidth, 0) + " (" + Utils.doubleToString(
  ...
  private static String printClusterings(Clusterer clusterer, String trainFileName,
      String testFileName, Range attributesToOutput) throws Exception {
    ...
    while (source.hasMoreElements(structure)) {
      ...
      if (forBatchPredictors != null) {
        ...
        try {
          ...
          text.append(
              i + " " + cnum + " " + attributeValuesString(inst, attributesToOutput) + "\n");
        } catch (Exception e) {
          /*
    ...
    if (forBatchPredictors != null) {
      ...
      for (double[] d : dists) {
        cnum = Utils.maxIndex(d);
        text.append(i + " " + cnum + " " + attributeValuesString(forBatchPredictors.instance(i),
            attributesToOutput) + "\n");
        i++;
      }
    }
  ...
  public void evaluateClusterer(Instances test, String testFileName, boolean outputModel)
    ...
    Instances forBatchPredictors = filter != null ? new Instances(filter.getOutputFormat(), 0)
    ...
    if (m_Clusterer instanceof BatchPredictor
        && ((BatchPredictor) m_Clusterer).implementsMoreEfficientBatchPrediction()) {
      double[][] dists =
          ((BatchPredictor) m_Clusterer).distributionsForInstances(forBatchPredictors);
      for (double[] d : dists) {
        cnum = Utils.maxIndex(d);
        clusterAssignments.add((double) cnum);
        instanceStats[cnum]++;
    ...
    for (i = 0; i < cc; i++) {
      if (instanceStats[i] > 0) {
        m_clusteringResults.append(
            Utils.doubleToString(i, clustFieldWidth, 0) + "      " + Utils.doubleToString(
                instanceStats[i], numInstFieldWidth, 0) + " (" + Utils.doubleToString(
                (instanceStats[i] / sum * 100.0), 3, 0) + "%)\n");
      }
    }
  ...
  @Override public boolean equals(Object obj) {
    ...
    return clusteringResults1.equals(clusteringResults2);
  }
