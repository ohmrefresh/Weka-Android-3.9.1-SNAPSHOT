...
public class ThresholdCurve implements RevisionHandler {
  ...
  public static int getThresholdInstance(Instances tcurve, double threshold) {
    ...
    if (!RELATION_NAME.equals(tcurve.relationName()) || (tcurve.numInstances() == 0) || (threshold
        < 0) || (threshold > 1.0)) {
      return -1;
    }
  ...
  public Instances getCurve(ArrayList<Prediction> predictions, int classIndex) {
    ...
    if ((predictions.size() == 0) || (((NominalPrediction) predictions.get(0)).distribution().length
        <= classIndex)) {
      return null;
    }
  ...
  private Instance makeInstance(TwoClassStats tc, double prob) {
    ...
    double ss = (tc.getTruePositive() + tc.getFalsePositive()) / (tc.getTruePositive()
        + tc.getFalsePositive()
        + tc.getTrueNegative()
        + tc.getFalseNegative());
    vals[count++] = ss;
