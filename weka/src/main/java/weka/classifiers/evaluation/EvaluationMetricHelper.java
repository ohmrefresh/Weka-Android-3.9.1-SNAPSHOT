...
public class EvaluationMetricHelper {
  ...
  protected double getBuiltinMetricValue(int metricIndex, int... classValIndex) throws Exception {
    ...
    switch (metricIndex) {
      ...
      case 17:
        return hasValIndex ? m_eval.truePositiveRate(classValIndex[0])
            : m_eval.weightedTruePositiveRate();
      case 18:
        return hasValIndex ? m_eval.falsePositiveRate(classValIndex[0])
            : m_eval.weightedFalsePositiveRate();
      case 19:
      ...
      case 22:
        return hasValIndex ? m_eval.matthewsCorrelationCoefficient(classValIndex[0])
            : m_eval.weightedMatthewsCorrelation();
      case 23:
   ...
   */
  protected double getPluginMetricValue(AbstractEvaluationMetric m, String statName,
      int... classValIndex) throws Exception {

    boolean hasValIndex = classValIndex != null && classValIndex.length == 1;

    if (m instanceof InformationRetrievalEvaluationMetric) {
      return hasValIndex ? ((InformationRetrievalEvaluationMetric) m).getStatistic(statName,
          classValIndex[0])
          : ((InformationRetrievalEvaluationMetric) m).getClassWeightedAverageStatistic(statName);
    }
