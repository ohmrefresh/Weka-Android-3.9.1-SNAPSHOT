...
public class CSV extends AbstractOutput {
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    result.addElement(
        new Option("\tWhether to use TAB as separator instead of comma.\n" + "\t(default: comma)",
            "use-tab", 0, "-use-tab"));
   ...
   */
  @Override protected void doPrintClassification(double[] dist, Instance inst, int index)
      throws Exception {
    ...
    if (inst.dataset().classAttribute().isNumeric()) {
      ...
      if (Utils.isMissingValue(predValue)) {
        ...
        append(m_Delimiter + ((int) predValue + 1) + ":" + inst.dataset()
            .classAttribute()
            .value((int) predValue));
      }
      // error?
   ...
   * @throws Exception if something goes wrong
   */
  @Override protected void doPrintClassification(Classifier classifier, Instance inst, int index)
      throws Exception {
