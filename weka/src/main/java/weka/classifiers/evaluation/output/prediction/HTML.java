...
public class HTML extends AbstractOutput {
   ...
   */
  protected void doPrintClassification(Classifier classifier, Instance inst, int index)
      throws Exception {
  ...
  protected void doPrintHeader() {
    ...
    if (m_Header.classAttribute().isNominal()) {
      if (m_OutputDistribution) {
        append(
            "<td>inst#</td><td>actual</td><td>predicted</td><td>error</td><td colspan=\"" + m_Header
                .classAttribute()
                .numValues() + "\">distribution</td>");
      } else {
  ...
  protected void doPrintClassification(double[] dist, Instance inst, int index) throws Exception {
    ...
    if (inst.dataset().classAttribute().isNumeric()) {
      ...
      append("<td>"
          ...
          + ":"
          + sanitize(inst.toString(inst.classIndex()))
          + "</td>");
      // predicted
      if (Utils.isMissingValue(predValue)) {
        append("<td>" + "?" + "</td>");
      } else {
        append("<td>" + ((int) predValue + 1) + ":" + sanitize(
            inst.dataset().classAttribute().value((int) predValue)) + "</td>");
      }
      // error?
      if (!Utils.isMissingValue(predValue) && !inst.classIsMissing() && ((int) predValue + 1
          != (int) inst.classValue() + 1)) {
        append("<td>" + "+" + "</td>");
      } else {
        append("<td>" + "&nbsp;" + "</td>");
      ...
      if (m_OutputDistribution) {
        ...
        if (Utils.isMissingValue(predValue)) {
          ...
          append(
              "<td align=\"right\">" + Utils.doubleToString(dist[(int) predValue], prec) + "</td>");
        }
      }
    }
