...
public class XML extends AbstractOutput {
  ...
  public final static String DTD = "<!"
      ...
      + " "
      + "("
      + Attribute.typeToString(Attribute.NUMERIC)
      + DTD_SEPARATOR
      + Attribute.typeToString(Attribute.DATE)
      + DTD_SEPARATOR
      + Attribute.typeToString(Attribute.NOMINAL)
      + DTD_SEPARATOR
      + Attribute.typeToString(Attribute.STRING)
      + DTD_SEPARATOR
      + Attribute.typeToString(Attribute.RELATIONAL)
      + ")"
      + " "
      + DTD_REQUIRED
      + ">\n"
  ...
  protected void doPrintHeader() {
    ...
    append("<"
        ...
        + ATT_NAME
        + "=\""
        + sanitize(m_Header.relationName())
        + "\">\n");
  }
      ...
      for (int i = 0; i < instance.numAttributes(); i++) {
        if (m_Attributes.isInRange(i) && i != instance.classIndex()) {
          text.append("      <"
              ...
              + ATT_NAME
              + "=\""
              + sanitize(instance.attribute(i).name())
              + "\""
              + " "
              + ATT_TYPE
              + "=\""
