  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option(
        "\tFull name of base classifier.\n" + "\t(default: " + defaultClassifierString() +
            ((defaultClassifierOptions().length > 0) ? " with options " + Utils.joinOptions(
                defaultClassifierOptions()) + ")" : ")"), "W", 1, "-W"));
  ...
  public void setOptions(String[] options) throws Exception {
    ...
    if (classifierName.length() > 0) {
      ...
      if (classifierOptions.length > 0) {
        ...
        setClassifier(
            AbstractClassifier.forName(defaultClassifierString(), defaultClassifierOptions()));
      }
    }
  }
