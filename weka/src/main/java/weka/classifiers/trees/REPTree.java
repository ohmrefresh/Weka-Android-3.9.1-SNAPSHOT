...
public class REPTree extends AbstractClassifier
    ...
    Sourcable, PartitionGenerator, Randomizable {
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tSet minimum number of instances per leaf " + "(default 2).", "M", 1,
            "-M <minimum number of instances>"));
    newVector.addElement(new Option("\tSet minimum numeric class variance proportion\n"
        ...
        "-V <minimum variance for split>"));
    newVector.addElement(
        new Option("\tNumber of folds for reduced error pruning " + "(default 3).", "N", 1,
            "-N <number of folds>"));
    newVector.addElement(
        new Option("\tSeed for random data shuffling (default 1).", "S", 1, "-S <seed>"));
    ...
    newVector.addElement(
        new Option("\tSpread initial count over all class values (i.e." + " don't use 1 per value)",
            "R", 0, "-R"));
   ...
   */
  @Override public void setSeed(int newSeed) {
  ...
  @Override public void buildClassifier(Instances data) throws Exception {
    ...
    if (data.classAttribute().isNumeric()) {
      trainVariance =
          m_Tree.singleVariance(classProbs[0], totalSumSquared, totalWeight) / totalWeight;
      classProbs[0] /= totalWeight;
    }

    // Build tree
    m_Tree.buildTree(sortedIndices, weights, train, totalWeight, classProbs,
        new Instances(train, 0), m_MinNum, m_MinVarianceProp * trainVariance, 0, m_MaxDepth);
  ...
  protected class Tree implements Serializable, RevisionHandler {
    ...
    protected double[] distributionForInstance(Instance instance) throws Exception {
      ...
      if (m_Attribute > -1) {
        ...
        if (instance.isMissing(m_Attribute)) {
          ...
          returnedDist =
              m_Successors[(int) instance.value(m_Attribute)].distributionForInstance(instance);
        } else {
    ...
    protected int toGraph(StringBuffer text, int num, Tree parent) throws Exception {
      ...
      if (m_Attribute == -1) {
        text.append("N"
            ...
            + " [label=\""
            + num
            + Utils.backQuoteChars(leafString(parent))
            + "\""
            + "shape=box]\n");
      } else {
        text.append("N"
        ...
        for (int i = 0; i < m_Successors.length; i++) {
          text.append(
              "N" + Integer.toHexString(Tree.this.hashCode()) + "->" + "N" + Integer.toHexString(
                  m_Successors[i].hashCode()) + " [label=\"");
    ...
    protected String leafString(Tree parent) throws Exception {
      ...
      if (m_Info.classAttribute().isNumeric()) {
        ...
        buffer.append(" ["
            + Utils.doubleToString(m_HoldOutDist[0], getNumDecimalPlaces())
            + "/"
            + Utils.doubleToString(avgError, getNumDecimalPlaces())
            + "]");
        ...
        return " : "
            ...
            + "/"
            + Utils.doubleToString((Utils.sum(m_Distribution) - m_Distribution[maxIndex]),
            getNumDecimalPlaces())
            + ")"
            + " ["
            + Utils.doubleToString(Utils.sum(m_HoldOutDist), getNumDecimalPlaces())
            + "/"
            + Utils.doubleToString((Utils.sum(m_HoldOutDist) - m_HoldOutDist[maxIndex]),
            getNumDecimalPlaces())
            + "]";
      }
    }
      ...
      try {
        ...
        if (m_Attribute == -1) {
          ...
          for (int i = 0; i < m_Successors.length; i++) {
            ...
            text.append(m_Info.attribute(m_Attribute).name() + " = " + m_Info.attribute(m_Attribute)
                .value(i));
          ...
          text.append(
              m_Info.attribute(m_Attribute).name() + " < " + Utils.doubleToString(m_SplitPoint,
                  getNumDecimalPlaces()));
     ...
     * @throws Exception if generation fails
     */
    protected void buildTree(int[][][] sortedIndices, double[][][] weights, Instances data,
        double totalWeight, double[] classProbs, Instances header, double minNum,
        double minVariance, int depth, int maxDepth) throws Exception {
      ...
      if ((totalWeight < (2 * minNum)) ||
          ...
          // Nominal case
          (data.classAttribute().isNominal() && Utils.eq(m_ClassProbs[Utils.maxIndex(m_ClassProbs)],
      ...
      if (data.classAttribute().isNominal()) {
        ...
        for (int i = 0; i < data.numAttributes(); i++) {
          if (i != data.classIndex()) {
            splits[i] = distribution(props, dists, i, sortedIndices[0][i], weights[0][i],
        ...
        for (int i = 0; i < data.numAttributes(); i++) {
          if (i != data.classIndex()) {
            splits[i] = numericDistribution(props, dists, i, sortedIndices[0][i], weights[0][i],
      ...
      if (Utils.gr(vals[m_Attribute], 0) && (count > 1)) {
        ...
        splitData(subsetIndices, subsetWeights, m_Attribute, m_SplitPoint, sortedIndices[0],
            weights[0], data);
        ...
        for (int i = 0; i < numAttVals; i++) {
          ...
          m_Successors[i].buildTree(subsetIndices[i], subsetWeights[i], data,
              attTotalSubsetWeights[i], attSubsetDists[i], header, minNum, minVariance, depth + 1,
              maxDepth);
     ...
     * @throws Exception if something goes wrong
     */
    protected void splitData(int[][][][] subsetIndices, double[][][][] subsetWeights, int att,
        double splitPoint, int[][] sortedIndices, double[][] weights, Instances data)
        throws Exception {
    ...
    protected double distribution(double[][] props, double[][][] dists, int att,
        int[] sortedIndices, double[] weights, double[][] subsetWeights, Instances data)
        throws Exception {
