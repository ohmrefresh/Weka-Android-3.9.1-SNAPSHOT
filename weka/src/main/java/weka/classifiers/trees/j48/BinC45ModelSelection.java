...
public class BinC45ModelSelection extends ModelSelection {
   ...
   */
  public BinC45ModelSelection(int minNoObj, Instances allData, boolean useMDLcorrection,
  ...
  @Override public final ClassifierSplitModel selectModel(Instances data) {
    ...
    try {
      ...
      while (enu.hasMoreElements()) {
        ...
        if ((attribute.isNumeric()) || (Utils.sm(attribute.numValues(),
      ...
      for (i = 0; i < data.numAttributes(); i++) {
        ...
        if (i != (data).classIndex()) {
          ...
          if (currentModel[i].checkModel()) {
            if ((data.attribute(i).isNumeric()) || (multiVal || Utils.sm(
                data.attribute(i).numValues(), (0.3 * m_allData.numInstances())))) {
      ...
      for (i = 0; i < data.numAttributes(); i++) {
        if ((i != (data).classIndex()) && (currentModel[i].checkModel())) {
          ...
          // implementation.
          if ((currentModel[i].infoGain() >= (averageInfoGain - 1E-3)) && Utils.gr(
              currentModel[i].gainRatio(), minResult)) {
