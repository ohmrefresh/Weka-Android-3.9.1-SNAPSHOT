...
public class C45Split extends ClassifierSplitModel {
  ...
  private void handleNumericAttribute(Instances trainInstances) throws Exception {
    ...
    while (next < firstMiss) {
      ...
      if (trainInstances.instance(next - 1).value(m_attIndex) + 1e-5 < trainInstances.instance(next)
        ...
        // values for criteria.
        if (Utils.grOrEq(m_distribution.perBag(0), minSplit) && Utils.grOrEq(
            m_distribution.perBag(1), minSplit)) {
    ...
    m_numSubsets = 2;
    m_splitPoint =
        (trainInstances.instance(splitIndex + 1).value(m_attIndex) + trainInstances.instance(
            splitIndex).value(m_attIndex)) / 2;
   ...
   * @throws Exception if something goes wrong
   */
  @Override public final double classProb(int classIndex, Instance instance, int theSubset)
      throws Exception {
   ...
   */
  @Override public final double codingCost() {
   ...
   */
  @Override public void resetDistribution(Instances data) throws Exception {
