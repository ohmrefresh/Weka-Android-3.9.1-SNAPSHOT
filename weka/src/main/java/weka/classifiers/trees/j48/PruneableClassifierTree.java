...
public class PruneableClassifierTree extends ClassifierTree {
   ...
   */
  public PruneableClassifierTree(ModelSelection toSelectLocModel, boolean pruneTree, int num,
      boolean cleanup, int seed) throws Exception {
  ...
  public void buildClassifier(Instances data) throws Exception {
    ...
    buildTree(data.trainCV(numSets, numSets - 1, random), data.testCV(numSets, numSets - 1),
        !m_cleanup);
    if (pruneTheTree) {
      prune();
    }
    ...
    if (m_isLeaf) {
      ...
      for (int i = 0; i < m_sons.length; i++)
        if (Utils.eq(localModel().distribution().perBag(i), 0)) {
          errors += m_test.perBag(i) - m_test.perClassPerBag(i, localModel().distribution().
              maxClass());
        } else {
