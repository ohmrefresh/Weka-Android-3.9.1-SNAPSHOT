...
public class ActiveHNode extends LeafNode implements LearningNode, Serializable {
  ...
  protected Map<String, ConditionalSufficientStats> m_nodeStats =
      new HashMap<String, ConditionalSufficientStats>();
  ...
  @Override public void updateNode(Instance inst) throws Exception {
    ...
    for (int i = 0; i < inst.numAttributes(); i++) {
      ...
      if (i != inst.classIndex()) {
        ...
        stats.update(inst.value(a), inst.classAttribute().value((int) inst.classValue()),
            inst.weight());
      }
    }
  }
