...
public class RuleNode extends AbstractClassifier {
  ...
  @Override public void buildClassifier(Instances data) throws Exception {
    ...
    m_isLeaf = (m_numInstances < m_splitNum) || (Rule.stdDev(m_classIndex, m_instances) < (
        m_globalDeviation
            * m_devFraction));
  ...
  @Override public String getRevision() {
    return RevisionUtils.extract("$Revision: 12886 $");
  }
  ...
  public String treeToString(int level) {
    ...
    if (!m_isLeaf) {
      ...
      if (m_instances.attribute(m_splitAtt).name().charAt(0) != '[') {
        text.append(
            m_instances.attribute(m_splitAtt).name() + " <= " + Utils.doubleToString(m_splitValue,
                1, 3) + " : ");
      } else {
      ...
      if (m_globalDeviation > 0.0) {
        text.append(" (" + m_numInstances + "/" + Utils.doubleToString(
        ...
        if (current.m_parent != null) {
          ...
          for (int i = 0; i < coeffsUsedByLinearModel.length; i++) {
            if (i != m_classIndex) {
              ...
              coefficients[i] +=
                  ((SMOOTHING_CONSTANT * coeffsUsedByLinearModel[i]) / (n + SMOOTHING_CONSTANT));
            }
          }
          // smooth in the intercept
          intercept += ((SMOOTHING_CONSTANT * current.m_parent.getModel().intercept()) / (n
              + SMOOTHING_CONSTANT));
  ...
  public void prune() throws Exception {
    ...
    if (m_isLeaf) {
      ...
      adjustedErrorModel =
          rmsModel * pruningFactor(m_numInstances, m_nodeModel.numParameters() + 1);
      ...
      if ((adjustedErrorModel <= adjustedErrorNode) || (adjustedErrorModel < (m_globalDeviation
          * 0.00001))) {
  ...
  protected void graphTree(StringBuffer text) {
    text.append("N" + m_id + (m_isLeaf ? " [label=\"LM " + m_leafModelNum
        : " [label=\"" + Utils.backQuoteChars(m_instances.attribute(m_splitAtt).name())) + (m_isLeaf
        ? " ("
        + ((m_globalDeviation > 0.0) ? m_numInstances + "/" + Utils.doubleToString(
        (100.0 * m_rootMeanSquaredError / m_globalDeviation), 1, 3) + "%)" : m_numInstances + ")")
        + "\" shape=box style=filled " : "\"") + (m_saveInstances ? "data=\n"
        + m_instances
        + "\n,\n" : "") + "]\n");

    if (m_left != null) {
      text.append("N" + m_id + "->" + "N" + m_left.m_id + " [label=\"<=" + Utils.doubleToString(
          m_splitValue, 1, 3) + "\"]\n");
      m_left.graphTree(text);
    }

    if (m_right != null) {
      text.append("N" + m_id + "->" + "N" + m_right.m_id + " [label=\">" + Utils.doubleToString(
          m_splitValue, 1, 3) + "\"]\n");
