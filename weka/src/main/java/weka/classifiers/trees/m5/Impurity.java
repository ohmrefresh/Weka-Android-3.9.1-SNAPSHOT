...
public final class Impurity implements RevisionHandler {
  ...
  int order;
      // order = 1, variance; order = 2, standard deviation; order = 3, the cubic root of the variance;
  // order = k, the k-th order root of the variance

  /**
   * Constructs an Impurity object containing the impurity values of partitioning the instances
   * using an attribute
   *
   ...
   * @param k the order of the impurity; =1, the variance; =2, the stardard deviation; =k, the k-th
   ...
   * @param type if type=1, value will be added to the left subset; type=-1, to the right subset;
   * type=0, initializes
   */
