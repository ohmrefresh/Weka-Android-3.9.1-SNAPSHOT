...
/**
 * <!-- globalinfo-start -->
 * Class for constructing a tree that considers K randomly  chosen attributes at each node. Performs
 * no pruning. Also has an option to allow estimation of class probabilities (or target mean in the
...
public class RandomTree extends AbstractClassifier
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tSeed for random number generator.\n" + "\t(default 1)", "S", 1, "-S <num>"));
    ...
    newVector.addElement(
        new Option("\tNumber of folds for backfitting " + "(default 0, no backfitting).", "N", 1,
            "-N <num>"));
   ...
   *
   * @param newAllowUnclassifiedInstances true if tree is allowed to abstain from making a
   * prediction
   */
  ...
  @Override public void buildClassifier(Instances data) throws Exception {
    ...
    if (data.classAttribute().isNumeric()) {
      trainVariance =
          RandomTree.singleVariance(classProbs[0], totalSumSquared, totalWeight) / totalWeight;
  ...
  protected class Tree implements Serializable {
    ...
    public int toGraph(StringBuffer text, int num) throws Exception {
      ...
      String classValue =
          m_Info.classAttribute().isNominal() ? m_Info.classAttribute().value(maxIndex)
              : Utils.doubleToString(m_ClassDistribution[0], getNumDecimalPlaces());
      ...
      if (m_Attribute == -1) {
        ...
        text.append("N"
            ...
            + "\"]\n");
        for (int i = 0; i < m_Successors.length; i++) {
          text.append("N" + Integer.toHexString(hashCode()) + "->" + "N" + Integer.toHexString(
              m_Successors[i].hashCode()) + " [label=\"" + m_Info.attribute(m_Attribute).name());
    ...
    protected String leafString() throws Exception {
      ...
      if (m_Info.classAttribute().isNumeric()) {
        return " : "
            + Utils.doubleToString(classMean, getNumDecimalPlaces())
            + " ("
    ...
    protected void buildTree(Instances data, double[] classProbs, int[] attIndicesWindow,
      ...
      while ((windowSize > 0) && (k-- > 0 || !gainFound)) {
        ...
        double currSplit =
            data.classAttribute().isNominal() ? distribution(props, dists, attIndex, data)
        ...
        if ((currVal > val) || ((!getBreakTiesRandomly()) && (currVal == val) && (attIndex
            < bestIndex))) {
          val = currVal;
          bestIndex = attIndex;
          split = currSplit;
    ...
    protected int toGraph(StringBuffer text, int num, Tree parent) throws Exception {
      ...
      if (m_Attribute == -1) {
        text.append("N"
            ...
            + " [label=\""
            + num
            + Utils.backQuoteChars(leafString())
            + "\""
            + " shape=box]\n");
      } else {
        text.append("N"
