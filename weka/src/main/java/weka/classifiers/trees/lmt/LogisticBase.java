...
public class LogisticBase extends AbstractClassifier implements WeightedInstancesHandler {
   ...
   * @throws Exception if something goes wrong
   */
  protected boolean performIteration(int iteration, double[][] trainYs, double[][] trainFs,
      double[][] probs, Instances trainNumeric) throws Exception {

    SimpleLinearRegression[] linearRegressionForEachClass =
        new SimpleLinearRegression[m_numClasses];
    ...
    for (int j = 0; j < m_numClasses; j++) {
      ...
      if (weightSum > 0) {
        ...
        if (m_weightTrimBeta > 0) {
          ...
          for (int i = weightsOrder.length - 1;
              (i >= 0) && (weightPercentage < (1 - m_weightTrimBeta)); i--) {
    ...
    for (int i = 0; i < m_numClasses; i++) {
      m_regressions[i][linearRegressionForEachClass[i].getAttributeIndex()].addModel(
          linearRegressionForEachClass[i]);
    }
  ...
  @Override public String toString() {
    ...
    for (int j = 0; j < m_numClasses; j++) {
      ...
      for (int i = 0; i < attributes[j].length; i++) {
        ...
        s.append(" * " + Utils.doubleToString(coefficients[j][attributes[j][i] + 1],
            2 + m_numDecimalPlaces, m_numDecimalPlaces));
