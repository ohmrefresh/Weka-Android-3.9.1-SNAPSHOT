...
public class LMTNode extends LogisticBase {
   ...
   */
  public LMTNode(ModelSelection modelSelection, int numBoostingIterations, boolean fastRegression,
      boolean errorOnProbabilities, int minNumInstances, double weightTrimBeta, boolean useAIC,
      NominalToBinary ntb, int numDecimalPlaces) {
  ...
  public void buildTree(Instances data, SimpleLinearRegression[][] higherRegressions,
      double totalInstanceWeight, double higherNumParameters, Instances numericDataHeader)
      throws Exception {
    ...
    if (m_numInstances > m_minNumInstances) {
      ...
      if (m_modelSelection instanceof ResidualModelSelection) {
        ...
        m_localModel =
            ((ResidualModelSelection) m_modelSelection).selectModel(m_train, dataZs, dataWs);
      } else {
    ...
    if (grow) {
      ...
      for (int i = 0; i < m_sons.length; i++) {
        m_sons[i] = new LMTNode(m_modelSelection, m_fixedNumIterations, m_fastRegression,
            m_errorOnProbabilities, m_minNumInstances, getWeightTrimBeta(), getUseAIC(),
            m_nominalToBinary, m_numDecimalPlaces);
  ...
  public String graph() throws Exception {
    ...
    if (m_isLeaf) {
      ...
      text.append(
          "N" + m_id + " [label=\"" + Utils.backQuoteChars(m_localModel.leftSide(m_train)) + "\" ");
      text.append("]\n");
      graphTree(text);
    }
  ...
  private void graphTree(StringBuffer text) throws Exception {
    ...
    for (int i = 0; i < m_sons.length; i++) {
      text.append("N" + m_id + "->" + "N" + m_sons[i].m_id + " [label=\"" + Utils.backQuoteChars(
      ...
      if (m_sons[i].m_isLeaf) {
        ...
        text.append("N" + m_sons[i].m_id + " [label=\"" + Utils.backQuoteChars(
