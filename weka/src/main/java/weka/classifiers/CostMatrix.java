...
public class CostMatrix implements Serializable, RevisionHandler {
    ...
    while ((line = lnr.readLine()) != null) {
      ...
      if (currentRow < 0) {
        ...
        for (int i = 0; i < m_size; i++) {
          if (!st.hasMoreTokens()) {
            throw new Exception(
                "Line " + lnr.getLineNumber() + ": too few matrix elements provided");
          }
   ...
   * @throws Exception if the data has no class or the matrix in
   * inappropriate.
   */
  public Instances applyCostMatrix(Instances data, Random random) throws Exception {
    ...
    if (replaceStrings(data)) {
      ...
      if (data.classAttribute().numValues() > 2) {
        ...
        for (int i = 0; i < data.numInstances(); i++) {
          ...
          double factor = 1.0;
          Object element =
              (classValIndex == 0) ? getCell(classValIndex, 1) : getCell(classValIndex, 0);
    ...
    for (int i = 0; i < data.numInstances(); i++) {
      weightOfInstances[i] =
          data.instance(i).weight() * weightFactor[(int) data.instance(i).classValue()];
    }
   ...
   * @param classProbs the class probability estimates.
   * @return the expected costs.
   * @throws Exception if the wrong number of class probabilities is
   * supplied.
   */
   ...
   * @throws Exception if cost matrix contains non-fixed costs
   */
   ...
   * @throws Exception if the matrix cannot be read correctly.
   */
   ...
   * @throws Exception if the value is not a double
   */
  ...
  @Override public String toString() {
    ...
    for (int i = 0; i < size(); i++) {
      for (int j = 0; j < size(); j++) {
        ...
        if (element instanceof Double) {
          text.append(" ")
              .append(Utils.doubleToString(((Double) element).doubleValue(), width,
                  (fractional ? 2 : 0)));
        } else {
