...
public class NaiveBayesMultinomialUpdateable extends NaiveBayesMultinomial
  ...
  @Override public void updateClassifier(Instance instance) throws Exception {
    ...
    for (int a = 0; a < instance.numValues(); a++) {
      ...
      if (m_probOfWordGivenClass[classIndex][instance.index(a)] < 0) {
        throw new Exception(
            "Can't have a negative conditional sum for attribute " + instance.index(a));
      }
    }
  }
  ...
  @Override public String toString() {
    ...
    for (int c = 0; c < m_numClasses; c++) {
      result.append(m_headerInfo.classAttribute().value(c))
          .append("\t")
    ...
    for (int w = 0; w < m_numAttributes; w++) {
      ...
      for (int c = 0; c < m_numClasses; c++) {
        result.append(Utils.padLeft(Double.toString(m_probOfWordGivenClass[c][w]), 11))
            .append("\t");
      }
