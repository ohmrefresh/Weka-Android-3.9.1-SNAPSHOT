...
public class GeneticSearch extends LocalScoreSearchAlgorithm {
  ...
  @Override protected void search(BayesNet bayesNet, Instances instances) throws Exception {
    ...
    for (int iRun = 0; iRun < m_nRuns; iRun++) {
      // create descendants
      BayesNetRepresentation[] descendantPopulation =
          new BayesNetRepresentation[getDescendantPopulationSize()];
      ...
      for (int i = 0; i < getPopulationSize(); i++) {
        ...
        if (m_bUseTournamentSelection) {
          ...
          if (descendantPopulation[iSelected2].getScore()
              > descendantPopulation[iSelected].getScore()) {
