...
public class VaryNode implements Serializable, RevisionHandler {
   ...
   */
  public void getCounts(int[] nCounts, int[] nNodes, int[] nOffsets, int iNode, int iOffset,
      ADNode parent, boolean bSubstract) {
    for (int iValue = 0; iValue < m_ADNodes.length; iValue++) {
      if (iValue != m_nMCV) {
        ...
        parent.getCounts(nCounts, nNodes, nOffsets, iNode + 1, iOffset + nOffsets[iNode] * iValue,
            bSubstract);
