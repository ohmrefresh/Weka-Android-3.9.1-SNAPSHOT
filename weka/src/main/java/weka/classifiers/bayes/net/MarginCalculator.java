...
public class MarginCalculator implements Serializable, RevisionHandler {
  ...
  void initialize(JunctionTreeNode[] jtNodes, int[] order, Set<Integer>[] cliques,
      Set<Integer>[] separators, int[] parentCliques) {
  ...
  JunctionTreeNode[] getJunctionTree(Set<Integer>[] cliques, Set<Integer>[] separators,
    ...
    for (int i = 0; i < nNodes; i++) {
      ...
      if (cliques[iNode] != null) {
        ...
        if (parentCliques[iNode] > 0) {
          ...
          JunctionTreeSeparator jts =
              new JunctionTreeSeparator(separators[iNode], bayesNet, jtns[iNode], parent);
    ...
    for (int i = 0; i < nNodes; i++) {
      ...
      if (cliques[iNode] != null && separators[iNode].size() > 0) {
        ...
        for (int j = 0; j < nNodes; j++) {
          int iNode2 = order[j];
          if (iNode != iNode2 && cliques[iNode2] != null && cliques[iNode2].containsAll(
              separators[iNode])) {
        ...
        if (bAdjacencyMatrix[iNode][iNode2]) {
          for (int k = j + 1; k < i; k++) {
            ...
            if (bAdjacencyMatrix[iNode][iNode3]) {
              // fill in
              if (m_debug && (!bAdjacencyMatrix[iNode2][iNode3]
                  || !bAdjacencyMatrix[iNode3][iNode2])) {
  ...
  public void setEvidence(int nNode, int iValue) throws Exception {
    ...
    int iJtNode = 0;
    while (iJtNode < jtNodes.length && (jtNodes[iJtNode] == null || !jtNodes[iJtNode].contains(
        nNode))) {
      iJtNode++;
    }
  ...
  public class JunctionTreeSeparator implements Serializable, RevisionHandler {
    ...
    JunctionTreeSeparator(Set<Integer> separator, BayesNet bayesNet, JunctionTreeNode childNode,
    ...
    public double[] update(JunctionTreeNode node) {
      ...
      for (int iPos = 0; iPos < node.m_nCardinality; iPos++) {
        ...
        int i = 0;
        values[i]++;
        while (i < node.m_nNodes.length && values[i] == m_bayesNet.getCardinality(
            node.m_nNodes[i])) {
          values[i] = 0;
          i++;
  ...
  public class JunctionTreeNode implements Serializable, RevisionHandler {
    ...
    public void initializeUp() {
      ...
      for (JunctionTreeNode element : m_children) {
        ...
        for (int iPos = 0; iPos < m_nCardinality; iPos++) {
          int iSepCPT =
              getCPT(separator.m_nNodes, separator.m_nNodes.length, values, order, m_bayesNet);
    ...
    public void initializeDown(boolean recursively) {
      if (m_parentSeparator == null) { // a root node
        ...
        for (int iPos = 0; iPos < m_nCardinality; iPos++) {
          int iSepCPT =
              getCPT(m_parentSeparator.m_nNodes, m_parentSeparator.m_nNodes.length, values, order,
                  m_bayesNet);
          int iNodeCPT = getCPT(m_nNodes, m_nNodes.length, values, order, m_bayesNet);
          if (m_parentSeparator.m_fiChild[iSepCPT] > 0) {
            m_P[iNodeCPT] *=
                m_parentSeparator.m_fiParent[iSepCPT] / m_parentSeparator.m_fiChild[iSepCPT];
          } else {
            m_P[iNodeCPT] = 0;
          }
          // update values
