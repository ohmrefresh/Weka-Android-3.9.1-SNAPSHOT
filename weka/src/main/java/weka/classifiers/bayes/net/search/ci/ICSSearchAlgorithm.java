...
public class ICSSearchAlgorithm extends CISearchAlgorithm {
  ...
  static public void main(String[] argv) {
    try {
      ...
      Instances instances =
          new Instances(new FileReader("C:\\eclipse\\workspace\\weka\\data\\contact-lenses.arff"));
        ...
        if (!edges[iNode1][iNode2]) { /* i nonadj j */
          for (int iNode3 = 0; iNode3 < maxn(); iNode3++) {
            if ((iNode3 != iNode1
                && iNode3 != iNode2
                && edges[iNode1][iNode3]
                && edges[iNode2][iNode3]) & (!sepsets[iNode1][iNode2].contains(iNode3))) {
  ...
  *for
  ...
  Z that
  separates the
  nodes.MaxCardinality determines
  *
  the maximum
  size of
  the set
