...
public class SimpleEstimator extends BayesNetEstimator {
  ...
  @Override public void updateClassifier(BayesNet bayesNet, Instance instance) throws Exception {
    for (int iAttribute = 0; iAttribute < bayesNet.m_Instances.numAttributes(); iAttribute++) {
      ...
      bayesNet.m_Distributions[iAttribute][(int) iCPT].addValue(instance.value(iAttribute),
          instance.weight());
    }
  } // updateClassifier
    ...
    for (int iClass = 0; iClass < nNumClasses; iClass++) {
      ...
      for (int iAttribute = 0; iAttribute < instances.numAttributes(); iAttribute++) {
        ...
        if (iAttribute == instances.classIndex()) {
          ...
          logfP += Math.log(bayesNet.m_Distributions[iAttribute][(int) iCPT].getProbability(
