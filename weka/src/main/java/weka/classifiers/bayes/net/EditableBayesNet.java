...
public class EditableBayesNet extends BayesNet {
   ...
   * @throws Exception when data sets are not compatible, e.g., a variable is
  ...
  public void setData(Instances instances) throws Exception {
    ...
    for (int iNode = 0; iNode < getNrOfNodes(); iNode++) {
      if (newInstances.attribute(iNode).numValues() != getCardinality(iNode)) {
        throw new Exception("Number of values of node [[["
            + getNodeName(iNode)
  ...
  void paste(String sXML, int mode) throws Exception {
    ...
    Document doc =
        factory.newDocumentBuilder().parse(new org.xml.sax.InputSource(new StringReader(sXML)));
    doc.normalize();
   ...
   */
  @Override public String toXMLBIF03() {
    if (m_Instances == null) {
    ...
    for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
      ...
      text.append("<PROPERTY>position = ("
          + getPositionX(iAttribute)
          + ","
          + getPositionY(iAttribute)
          + ")</PROPERTY>\n");
  ...
  public String toXMLBIF03(ArrayList<Integer> nodes) {
    ...
    for (int iNode = 0; iNode < nodes.size(); iNode++) {
      ...
      for (int iParent = 0; iParent < m_ParentSets[nNode].getNrOfParents(); iParent++) {
        text.append("<GIVEN>" + XMLNormalize(
            m_Instances.attribute(m_ParentSets[nNode].getParent(iParent)).name()) + "</GIVEN>\n");
      }
      text.append("<TABLE>\n");
  ...
  class SetDistributionAction extends UndoAction {
    ...
    @Override public String getUndoMsg() {
      return "Distribution of node " + getNodeName(m_nTargetNode) + " changed";
    }

    @Override public String getRedoMsg() {
      return "Distribution of node " + getNodeName(m_nTargetNode) + " changed";
    }
