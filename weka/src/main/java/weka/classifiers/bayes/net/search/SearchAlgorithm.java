...
public class SearchAlgorithm implements OptionHandler, Serializable, RevisionHandler {
   ...
   */
  protected boolean addArcMakesSense(BayesNet bayesNet, Instances instances, int iAttributeHead,
      int iAttributeTail) {
    ...
    for (int iNode = 0; iNode < nNodes; iNode++) {
      ...
      for (int iNode2 = 0; !bFound && iNode2 < nNodes; iNode2++) {
        if (!bDone[iNode2]) {
          ...
          for (int iParent = 0; iParent < bayesNet.getParentSet(iNode2).getNrOfParents();
              iParent++) {
  ...
  protected boolean isArc(BayesNet bayesNet, int iAttributeHead, int iAttributeTail) {
    for (int iParent = 0; iParent < bayesNet.getParentSet(iAttributeHead).getNrOfParents();
        iParent++) {
   ...
   */
  protected boolean reverseArcMakesSense(BayesNet bayesNet, Instances instances, int iAttributeHead,
      int iAttributeTail) {
    ...
    if (m_sInitalBIFFile != null && !m_sInitalBIFFile.equals("")) {
      ...
      for (int iAttribute = 0; iAttribute < instances.numAttributes(); iAttribute++) {
        ...
        for (int iParent = 0; iParent < initialNet.getNrOfParents(iAttribute); iParent++) {
          ...
          int nParent = 0;
          while (nParent < bayesNet.getNrOfNodes() && !bayesNet.getNodeName(nParent)
              .equals(sParent)) {
            nParent++;
          }
  ...
  protected void doMarkovBlanketCorrection(BayesNet bayesNet, Instances instances) {
    ...
    for (int iAttribute = 0; iAttribute < instances.numAttributes(); iAttribute++) {
      ...
      for (int iAttribute2 = 0; !bIsInMarkovBoundary && iAttribute2 < instances.numAttributes();
          iAttribute2++) {
        bIsInMarkovBoundary =
            bayesNet.getParentSet(iAttribute2).contains(iAttribute) && bayesNet.getParentSet(
                iAttribute2).contains(iClass);
      }
