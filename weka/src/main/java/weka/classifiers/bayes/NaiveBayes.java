...
public class NaiveBayes extends AbstractClassifier
  ...
  @Override public String toString() {
    ...
    if (m_Instances == null) {
      ...
      for (int i = 0; i < m_Instances.numClasses(); i++) {
        String priorP =
            Utils.doubleToString(m_ClassDistribution.getProbability(i), maxWidth, 2).trim();
        priorP = "(" + priorP + ")";
      ...
      temp.append("\n");
      temp.append(pad("", "=",
          maxAttWidth + (maxWidth * m_Instances.numClasses()) + m_Instances.numClasses() + 1,
          true));
      temp.append("\n");
  ...
  public NaiveBayes aggregate(NaiveBayes toAggregate) throws Exception {
    ...
    if (m_UseDiscretization || toAggregate.getUseSupervisedDiscretization()) {
      throw new Exception(
          "Unable to aggregate when supervised discretization " + "has been turned on");
    }

    if (!m_Instances.equalHeaders(toAggregate.m_Instances)) {
      throw new Exception(
          "Can't aggregate - data headers don't match: " + m_Instances.equalHeadersMsg(
              toAggregate.m_Instances));
    }
