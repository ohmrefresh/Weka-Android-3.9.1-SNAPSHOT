 ...
 */
public class BayesNet extends AbstractClassifier
  ...
  protected Instances normalizeDataSet(Instances instances) throws Exception {
    ...
    if ((m_nNonDiscreteAttribute > -1) && (instances.attribute(m_nNonDiscreteAttribute).type()
        != Attribute.NOMINAL)) {
  ...
  protected Instance normalizeInstance(Instance instance) throws Exception {
    if ((m_nNonDiscreteAttribute > -1) && (instance.attribute(m_nNonDiscreteAttribute).type()
        != Attribute.NOMINAL)) {
  ...
  @Override public void setOptions(String[] options) throws Exception {
    ...
    if (searchAlgorithmName.length() != 0) {
      setSearchAlgorithm((SearchAlgorithm) Utils.forName(SearchAlgorithm.class, searchAlgorithmName,
    ...
    if (estimatorName.length() != 0) {
      setEstimator((BayesNetEstimator) Utils.forName(BayesNetEstimator.class, estimatorName,
    ...
    for (int iClass = 0; iClass < m_NumClasses; iClass++) {
      ...
      for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
        ...
        if (iAttribute == m_Instances.classIndex()) {
          fCount +=
              ((DiscreteEstimatorBayes) m_Distributions[iAttribute][(int) iCPT]).getCount(iClass);
        } else {
          fCount += ((DiscreteEstimatorBayes) m_Distributions[iAttribute][(int) iCPT]).getCount(
  ...
  @Override public String toString() {
    ...
    if (m_Instances == null) {
      ...
      for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
        text.append(
            m_Instances.attribute(iAttribute).name() + "(" + m_Instances.attribute(iAttribute)
                .numValues() + "): ");
  ...
  public String toXMLBIF03() {
    ...
    for (int iAttribute = 0; iAttribute < m_Instances.numAttributes(); iAttribute++) {
      ...
      text.append("<TABLE>\n");
      for (int iParent = 0; iParent < m_ParentSets[iAttribute].getCardinalityOfParents();
          iParent++) {
        for (int iValue = 0; iValue < m_Instances.attribute(iAttribute).numValues(); iValue++) {
   ...
   */
  @Override public double getMeasure(String measureName) {
