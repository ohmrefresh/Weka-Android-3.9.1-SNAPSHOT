...
/**
 * <!-- globalinfo-start -->
 * This class performs Bias-Variance decomposion on any classifier using the sub-sampled
 * cross-validation procedure as specified in (1).<br/>
 ...
 * <br/>
 * Geoffrey I. Webb, Paul Conilione (2002). Estimating bias and variance from data. School of
 * Computer Science and Software Engineering, Victoria, Australia.<br/>
 * <br/>
 * Ron Kohavi, David H. Wolpert: Bias Plus Variance Decomposition for Zero-One Loss Functions. In:
 * Machine Learning: Proceedings of the Thirteenth International Conference, 275-283, 1996.<br/>
 * <br/>
...
public class BVDecomposeSegCVSub
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tThe index of the class attribute.\n" + "\t(default last)", "c", 1,
            "-c <class index>"));
    newVector.addElement(new Option("\tTurn on debugging output.", "D", 0, "-D"));
    newVector.addElement(
        new Option("\tThe number of times each instance is classified.\n" + "\t(default 10)", "l",
            1, "-l <num>"));
    newVector.addElement(
        new Option("\tThe average proportion of instances common between any two training sets",
            "p", 1, "-p <proportion of objects in common>"));
    newVector.addElement(new Option("\tThe random number seed used.", "s", 1, "-s <seed>"));
    newVector.addElement(
        new Option("\tThe name of the arff file used for the decomposition.", "t", 1,
            "-t <name of arff file>"));
    newVector.addElement(new Option("\tThe number of instances in the training set.", "T", 1,
  ...
  public void decompose() throws Exception {
    ...
    } else if (m_TrainSize < 0
        || m_TrainSize >= data.numInstances() - 1) {  // Check if 0 < training Size < D - 1
    ...
    } else if (m_P < (m_TrainSize / ((double) data.numInstances() - 1))
        || m_P >= 1.0) { //Check if p is in range: m/(|D|-1) <= p < 1.0
      throw new Exception("Proportion is not in range: "
          + (m_TrainSize / ((double) data.numInstances() - 1))
          + " <= p < 1.0 ");
    }

    //roundup tps from double to integer
    tps = (int) Math.ceil(((double) m_TrainSize / m_P) + 1);
    ...
    for (int l = 0; l < m_ClassifyIterations; l++) {
      ...
      for (int i = 1; i <= q; i++) {
        ...
        int[] currentSegment = segmentList.get(i - 1);
        ...
        for (int j = 1; j <= k; j++) {
          ...
          for (int foldNum = 1; foldNum <= k; foldNum++) {
            if (foldNum != j) {
              ...
              for (int currentFoldIndex = startFoldIndex; currentFoldIndex <= endFoldIndex;
                  currentFoldIndex++) {
          ...
          if (getTrainSize() > TP.numInstances()) {
            throw new Exception("The training set size of "
                + getTrainSize()
                + ", is greater than the training pool "
                + TP.numInstances());
          }
          ...
          if (i == 1 && j == 1) {
            int[] segmentElast = segmentList.lastElement();
    ...
    for (int i = 0; i < data.numInstances(); i++) {
      ...
      for (int j = 0; j < numClasses; j++) {
        ...
        bsum += (pActual - pPred) * (pActual - pPred) - pPred * (1 - pPred) / (m_ClassifyIterations
            - 1);
        vsum += pPred * pPred;
      ...
      for (int count = 0; count < centralTendencies.size(); count++) {
        ...
        int wB = 0, wV = 0;
        int centralTendency = centralTendencies.get(count).intValue();
  ...
  public String globalInfo() {
    return "This class performs Bias-Variance decomposion on any classifier using the "
  ...
  public TechnicalInformation getTechnicalInformation() {
    ...
    result.setValue(Field.ADDRESS,
        "School of Computer Science and Software Engineering, Victoria, Australia");
