...
/**
 * <!-- globalinfo-start -->
 * * Implements Gaussian processes for regression without hyperparameter-tuning. To make choosing an
 * appropriate noise level easier, this implementation applies normalization/standardization to the
 * target attribute as well as the other attributes (if  normalization/standardizaton is turned on).
 * Missing values are replaced by the global mean/mode. Nominal attributes are converted to binary
 * ones. Note that kernel caching is turned off if the kernel used implements CachedKernel.
 * * <br><br>
 * <!-- globalinfo-end -->
 *
 ...
 */
public class GaussianProcesses extends RandomizableClassifier
    implements IntervalEstimator, ConditionalDensityEstimator, TechnicalInformationHandler,
  ...
  @Override public void buildClassifier(Instances insts) throws Exception {
    ...
    for (int i = 0; i < n; i++) {
      ...
      m_L.set(i, i, m_weights[i] * m_weights[i] * m_actualKernel.eval(i, i, insts.instance(i))
          + m_deltaSquared);
    }
   ...
   */
  @Override public double[][] predictIntervals(Instance inst, double confidenceLevel)
      throws Exception {
   ...
   */
  @Override public double logDensity(Instance inst, double value) throws Exception {
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    result.addElement(
        new Option("\tLevel of Gaussian Noise wrt transformed target." + " (default 1)", "L", 1,
            "-L <double>"));
  ...
  @Override public String[] getOptions() {
    ...
    result.addElement("-K");
    result.addElement(
        "" + m_kernel.getClass().getName() + " " + Utils.joinOptions(m_kernel.getOptions()));
  ...
  public String noiseTipText() {
    return
        "The level of Gaussian Noise (added to the diagonal of the Covariance Matrix), after the "
