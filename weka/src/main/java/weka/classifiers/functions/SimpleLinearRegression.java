...
public class SimpleLinearRegression extends AbstractClassifier implements WeightedInstancesHandler {
  ...
  @Override public void buildClassifier(Instances insts) throws Exception {
    ...
    for (int i = 0; i < insts.numAttributes(); i++) {
      ...
      double sseForMissing =
          classSumSquaredForMissing[i] - (classSumForMissing[i] * classMeanForMissing[i]);
    ...
    if (chosen == -1) {
      ...
      if (m_outputAdditionalStats) {
        ...
        double[] stdErrors =
            RegressionAnalysis.calculateStdErrorOfCoef(insts, m_attribute, m_slope, m_intercept,
                m_df);
        ...
        m_rsquaredAdj =
            RegressionAnalysis.calculateAdjRSquared(m_rsquared, insts.numInstances(), 2);
  ...
  @Override public String toString() {
    ...
    if (m_attribute == null) {
      ...
      if (m_outputAdditionalStats) {
        ...
        text.append(
            Utils.padRight("\nconst", attNameLength + 1) + Utils.doubleToString(m_intercept, 12,
                4));
