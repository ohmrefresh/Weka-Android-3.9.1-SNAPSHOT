 ...
 * <pre>
 * &#64;article{Lodhi2002,
 *    author = {Huma Lodhi and Craig Saunders and John Shawe-Taylor and Nello Cristianini and
...
public class StringKernel extends Kernel implements TechnicalInformationHandler {
  ...
  public static final Tag[] TAGS_PRUNING = {
      new Tag(PRUNING_NONE, "No pruning"), new Tag(PRUNING_LAMBDA, "Lambda pruning"),
  };
  /**
   ...
   */
  public StringKernel(Instances data, int cacheSize, int subsequenceLength, double lambda,
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    for (i = 0; i < TAGS_PRUNING.length; i++) {
      ...
      desc +=
          "\t" + tag.getSelectedTag().getID() + " = " + tag.getSelectedTag().getReadable() + "\n";
    }

    result.addElement(
        new Option("\tThe pruning method to use:\n" + desc + "\t(default: " + PRUNING_NONE + ")",
            "P", 1, "-P <" + param + ">"));

    result.addElement(
        new Option("\tThe size of the cache (a prime number).\n" + "\t(default: 250007)", "C", 1,
            "-C <num>"));

    result.addElement(
        new Option("\tThe size of the internal cache (a prime number).\n" + "\t(default: 200003)",
            "IC", 1, "-IC <num>"));
    ...
    result.addElement(
        new Option("\tThe maximum length of the subsequence.\n" + "\t(default: 9)", "ssl-max", 1,
            "-ssl-max <num>"));
  ...
  protected double kernelHelper(int n, char[] s, int endIndexS, char[] t, int endIndexT) {
    ...
    result =
        m_lambda * kernelHelper(n, s, endIndexS - 1, t, endIndexT) + kernelHelper2(n, s, endIndexS,
            t, endIndexT);
    if (m_useRecursionCache) {
  ...
  protected double kernelHelper2(int n, char[] s, int endIndexS, char[] t, int endIndexT) {
    ...
    if (x == t[endIndexT]) {
      double ret =
          m_lambda * (kernelHelper2(n, s, endIndexS, t, endIndexT - 1) + m_lambda * kernelHelper(
              n - 1, s, endIndexS - 1, t, endIndexT - 1));
      if (m_useRecursionCache) {
   ...
   * according to and depending on the initial value for n.
   */
  protected double kernelLP(int n, char[] s, int endIndexS, char[] t, int endIndexT,
   ...
   * @return a partial result for K
   */
  protected double kernelHelperLP(int n, char[] s, int endIndexS, char[] t, int endIndexT,
      int remainingMatchLength) {
    ...
    if (m_useRecursionCache) {
      adr =
          m_multX * n + m_multY * endIndexS + m_multZ * endIndexT + m_multZZ * remainingMatchLength;
      if (cachekh2K[adr % maxCache] == adr + 1) {
        return cachekh2[adr % maxCache];
      }
    }
   ...
   * @return a partial result for K'
   */
  protected double kernelHelper2LP(int n, char[] s, int endIndexS, char[] t, int endIndexT,
      int remainingMatchLength) {
    ...
    if (x == t[endIndexT]) {
      double ret =
          m_lambda * (kernelHelper2LP(n, s, endIndexS, t, endIndexT - 1, remainingMatchLength - 1)
              + m_lambda * kernelHelperLP(n - 1, s, endIndexS - 1, t, endIndexT - 1,
    ...
    for (i = endIndexT; i >= minIndex; i--) {
      if (x == t[i]) {
        ...
        double ret = getPowerOfLambda(skipLength) * kernelHelper2LP(n, s, endIndexS, t, i,
            remainingMatchLength - skipLength);
        if (m_useRecursionCache && endIndexS >= 0 && endIndexT >= 0 && n >= 0) {
          cachekh2K[adr % maxCache] = adr + 1;
          cachekh2[adr % maxCache] = ret;
        }
