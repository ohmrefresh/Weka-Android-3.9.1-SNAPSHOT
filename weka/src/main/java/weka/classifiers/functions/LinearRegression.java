...
public class LinearRegression extends AbstractClassifier
  ...
  public static final Tag[] TAGS_SELECTION = {
      new Tag(SELECTION_NONE, "No attribute selection"), new Tag(SELECTION_M5, "M5 method"),
  ...
  @Override public void buildClassifier(Instances data) throws Exception {
    ...
    if (m_outputAdditionalStats) {
      ...
      m_RSquaredAdj =
          RegressionAnalysis.calculateAdjRSquared(m_RSquared, m_TransformedData.numInstances(), k);
      ...
      m_StdErrorOfCoef =
          RegressionAnalysis.calculateStdErrorOfCoef(m_TransformedData, m_SelectedAttributes, se,
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tDo not try to eliminate colinear" + " attributes.\n", "C", 0, "-C"));

    newVector.addElement(
        new Option("\tSet ridge parameter (default 1.0e-8).\n", "R", 1, "-R <double>"));
  ...
  @Override public String getRevision() {
    return RevisionUtils.extract("$Revision: 13013 $");
  }
      ...
      if (m_outputAdditionalStats) {
        ...
        for (int i = 0; i < m_TransformedData.numAttributes(); i++) {
          if ((i != m_ClassIndex) && (m_SelectedAttributes[i])) {
            ...
            text.append(Utils.doubleToString(m_Coefficients[column], 12, m_numDecimalPlaces));
            text.append(
                "   " + Utils.doubleToString(m_StdErrorOfCoef[column], 12, m_numDecimalPlaces));
        ...
        text.append(Utils.padRight("\nconst", maxAttLength + 1) + Utils.doubleToString(
            m_Coefficients[column], 12, m_numDecimalPlaces));
  ...
  protected boolean deselectColinearAttributes(boolean[] selectedAttributes,
    ...
    if (maxAttr >= 0) {
      ...
      if (m_Debug) {
        System.out.println("Deselected colinear attribute:"
            + (maxAttr + 1)
            + " with standardised coefficient: "
            + maxSC);
      }
      return true;
    }
  ...
  protected void findBestModel() throws Exception {
    ...
    } while (m_EliminateColinearAttributes && deselectColinearAttributes(m_SelectedAttributes,
        m_Coefficients));
          ...
          for (int i = 0; i < m_SelectedAttributes.length; i++) {
            if (currentSelected[i]) {
              ...
              double currentAkaike =
                  currentMSE / fullMSE * (numInstances - numAttributes) + 2 * currentNumAttributes;
              if (m_Debug) {
  ...
  protected double[] doRegression(boolean[] selectedAttributes) throws Exception {
    ...
    if (numAttributes > 0) {
      ...
      System.arraycopy(((DenseVector) coeffsWithoutIntercept).getData(), 0, coefficients, 0,
          numAttributes);
    }
