...
/**
 * <!-- globalinfo-start -->
 * K* is an instance-based classifier, that is the class of a test instance is based upon the class
 * of those training instances similar to it, as determined by some similarity function.  It differs
 * from other instance-based learners in that it uses an entropy-based distance function.<br/>
 * <br/>
 ...
 */
public class KStar extends AbstractClassifier
  ...
  public Enumeration<Option> listOptions() {
    ...
    optVector.addElement(
        new Option("\tEnable entropic auto-blend setting (symbolic class only)\n", "E", 0, "-E"));
  ...
  private double attrTransProb(Instance first, Instance second, int col) {
    ...
    switch (m_Train.attribute(col).type()) {
      case Attribute.NOMINAL:
        ksNominalAttr =
            new KStarNominalAttribute(first, second, col, m_Train, m_RandClassCols, m_Cache[col]);
      ...
      case Attribute.NUMERIC:
        ksNumericAttr =
            new KStarNumericAttribute(first, second, col, m_Train, m_RandClassCols, m_Cache[col]);
