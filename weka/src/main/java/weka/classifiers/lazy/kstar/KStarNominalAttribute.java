...
public class KStarNominalAttribute implements KStarConstants, RevisionHandler {
   ...
   * Constructor
   */
  public KStarNominalAttribute(Instance test, Instance train, int attrIndex, Instances trainSet,
      int[][] randClassCol, KStarCache cache) {
    m_Test = test;
    m_Train = train;
  ...
  private double stopProbUsingEntropy() {
    ...
    if (upvals.avgProb == 0) {
      ...
      if ((upvals.randEntropy - upvals.actEntropy < botvals.randEntropy - botvals.actEntropy) && (
  ...
  private double stopProbUsingBlend() {
    ...
    aimfor = (m_TotalCount - m_Distribution[testvalue]) * (double) m_BlendFactor / 100.0
        + m_Distribution[testvalue];
