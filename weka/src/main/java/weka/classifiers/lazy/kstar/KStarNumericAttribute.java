...
public class KStarNumericAttribute implements KStarConstants, RevisionHandler {
   ...
   * Constructor
   */
  public KStarNumericAttribute(Instance test, Instance train, int attrIndex, Instances trainSet,
      int[][] randClassCols, KStarCache cache) {
    m_Test = test;
    m_Train = train;
  ...
  private double scaleFactorUsingEntropy() {
    ...
    for (j = 0; j < m_NumInstances; j++) {
      if (m_TrainSet.instance(j).isMissing(m_AttrIndex)) {
        ...
        m_Distances[j] =
            Math.abs(m_TrainSet.instance(j).value(m_AttrIndex) - m_Test.value(m_AttrIndex));
