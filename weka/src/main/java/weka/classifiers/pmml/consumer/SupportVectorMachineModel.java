...
public class SupportVectorMachineModel extends PMMLClassifier implements Serializable {
  ...
  *
  ...
  protected classificationMethod m_classificationMethod = classificationMethod.NONE;
      // PMML 3.x (only handles binary problems)
  ...
  public SupportVectorMachineModel(Element model, Instances dataDictionary,
    ...
    for (int i = 0; i < machineL.getLength(); i++) {
      ...
      SupportVectorMachine newMach =
          new SupportVectorMachine((Element) machine, m_miningSchema, m_vectorDictionary,
              m_svmRepresentation, m_alternateBinaryTargetCategory, m_log);
  ...
  public double[] distributionForInstance(Instance inst) throws Exception {
    ...
    if (hasMissing) {
      if (!m_miningSchema.hasTargetMetaData()) {
        String message =
            "[SupportVectorMachineModel] WARNING: Instance to predict has missing value(s) but "
                ...
                ? "zero probabilities output)." : "NaN output).");
        if (m_log == null) {
      ...
      for (SupportVectorMachine m : m_machines) {
        m.distributionForInstance(incoming, m_kernel, m_vectorDictionary, preds,
            m_classificationMethod, m_threshold);
      }
    }

    if (m_classificationMethod != classificationMethod.NONE && m_miningSchema.getFieldsAsInstances()
        .classAttribute()
        .isNominal()) {
      // PMML 4.0
    ...
    if (m_machines.size() == preds.length - 1) {
      ...
      if (total > 1.0) {
        throw new Exception(
            "[SupportVectorMachineModel] total of probabilities" + " is greater than 1!");
      }
...
*
  ...
  *
  ...
  class implementing a single binary
  ...
  */
  ...
  static class SupportVectorMachine implements Serializable {
     ...
     * @throws Exception if something goes wrong
     */
    public SupportVectorMachine(Element machineElement, MiningSchema miningSchema,
        VectorDictionary dictionary, SVM_representation svmRep, int altCategoryInd, Logger log)
        throws Exception {
      ...
      if (coefficientsL.getLength() != 1) {
        throw new Exception(
            "[SupportVectorMachine] Should be just one list of " + "coefficients per binary SVM!");
      }
     ...
     * @throws Exception if something goes wrong
     */
    public void distributionForInstance(double[] input, Kernel kernel, VectorDictionary vecDict,
        double[] preds, classificationMethod cMethod, double globalThreshold) throws Exception {
      int targetIndex = 0;
...
/*      if (result < 0 && m_miningSchema.getFieldsAsInstances().classAttribute().isNominal()) {
      ...
      if (cMethod == classificationMethod.NONE || m_miningSchema.getFieldsAsInstances()
          .classAttribute()
          .isNumeric()) {
        // ----------------------------------------------------------------------
        ...
        if (cMethod == classificationMethod.ONE_AGAINST_ALL) {
          ...
          // one-vs-one
          double threshold =
              (m_localThreshold < Double.MAX_VALUE) ? m_localThreshold : globalThreshold;
          ...
          if (result < threshold) {
            ...
            int altCat =
                (m_localAlternateTargetCategoryIndex != -1) ? m_localAlternateTargetCategoryIndex
    ...
    public String toString() {
      ...
      if (m_miningSchema.getFieldsAsInstances().classAttribute().isNominal()) {
        ...
        if (m_localAlternateTargetCategoryIndex != -1) {
          temp.append(
              "\n (alternate category = " + m_miningSchema.getFieldsAsInstances().classAttribute().
