...
public class TreeModel extends PMMLClassifier implements Drawable {
  ...
  public TreeModel(Element model, Instances dataDictionary, MiningSchema miningSchema)
      throws Exception {
    ...
    if (missingP != null && missingP.length() > 0) {
      ...
      try {
        ...
        System.err.println(
            "[TreeModel] WARNING: " + "couldn't parse supplied missingValuePenalty as a number");
      }
    }
  ...
  static class True extends Predicate {
    ...
    @Override public Predicate.Eval evaluate(double[] input) {
  ...
  static class SimplePredicate extends Predicate {
    ...
    public SimplePredicate(Element simpleP, MiningSchema miningSchema) throws Exception {
      ...
      if (att == null) {
        throw new Exception("[SimplePredicate] unable to find field "
            + fieldS
      ...
      if (m_operator != Operator.ISMISSING && m_operator != Operator.ISNOTMISSING) {
        ...
        if (att.isNumeric()) {
          ...
          if (m_value < 0) {
            throw new Exception(
                "[SimplePredicate] can't find value " + valueS + " in nominal " + "attribute " + att
                    .name());
          }
        }
      }
  ...
  static class CompoundPredicate extends Predicate {
    ...
    enum BooleanOperator {
      OR("or") {
        @Override Predicate.Eval evaluate(ArrayList<Predicate> constituents, double[] input) {
    ...
    @Override public String toString(int level, boolean cr) {
      StringBuffer text = new StringBuffer();
      for (int j = 0; j < level; j++) {
        text.append("|   ");
      }
  ...
  static class SimpleSetPredicate extends Predicate {
    ...
    public SimpleSetPredicate(Element setP, MiningSchema miningSchema) throws Exception {
      ...
      if (m_set == null) {
        throw new Exception(
            "[SimpleSetPredictate] couldn't find an " + "array containing the set values!");
      }
    ...
    enum BooleanOperator {
      IS_IN("isIn") {
        @Override Predicate.Eval evaluate(double[] input, int fieldIndex, Array set,
      ...
      abstract Predicate.Eval evaluate(double[] input, int fieldIndex, Array set,
          Attribute nominalLookup);
  ...
  class TreeNode implements Serializable {
    ...
    private final ArrayList<ScoreDistribution> m_scoreDistributions =
        ...
        if (child.getNodeType() == Node.ELEMENT_NODE) {
          ...
          if (tagName.equals("ScoreDistribution")) {
            ScoreDistribution newDist =
                new ScoreDistribution((Element) child, miningSchema, m_recordCount);
     ...
     * @throws Exception if something goes wrong.
     */
    protected double[] missingValueStrategyWeightedConfidence(double[] instance, Attribute classAtt)
        throws Exception {

      if (classAtt.isNumeric()) {
        throw new Exception(
            "[TreeNode] missing value strategy weighted confidence, " + "but class is numeric!");
      }
    ...
    protected double[] freqCountsForAggNodesStrategy(double[] instance, Attribute classAtt)
        throws Exception {
      ...
      if (m_childNodes.size() > 0) {
        ...
        if (m_scoreDistributions.size() == 0) {
          throw new Exception("[TreeModel] missing value strategy aggregate nodes:"
              + " no score distributions at leaf "
              + m_ID);
        }
        for (ScoreDistribution s : m_scoreDistributions) {
     ...
     * @throws Exception if something goes wrong.
     */
    protected double[] missingValueStrategyAggregateNodes(double[] instance, Attribute classAtt)
        throws Exception {

      if (classAtt.isNumeric()) {
        throw new Exception(
            "[TreeNode] missing value strategy aggregate nodes, " + "but class is numeric!");
      }
     ...
     * @throws Exception if something goes wrong.
     */
    protected double[] missingValueStrategyDefaultChild(double[] instance, Attribute classAtt)
        throws Exception {
        ...
        if (!strategyInvoked) {
          ...
          if (m_defaultChild != null) {
            ...
            throw new Exception("[TreeNode] missing value strategy is defaultChild, but "
                + "no default child has been specified in node "
                + m_ID);
          }
        }
      }
     ...
     * @throws Exception if something goes wrong.
     */
    protected double[] missingValueStrategyLastPrediction(double[] instance, Attribute classAtt)
        throws Exception {
     ...
     * @throws Exception if something goes wrong.
     */
    protected double[] missingValueStrategyNullPrediction(double[] instance, Attribute classAtt)
        throws Exception {
