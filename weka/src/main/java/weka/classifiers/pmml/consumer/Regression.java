...
public class Regression extends PMMLClassifier implements Serializable {
  ...
  */
    ...
    for (int i = 0; i < tableList.getLength(); i++) {
      ...
      if (table.getNodeType() == Node.ELEMENT_NODE) {
        RegressionTable tempRTable =
            new RegressionTable((Element) table, functionType, m_miningSchema);
  ...
  /**
   * Rnum for the normalization methods.
   */
  enum Normalization {
  ...
  /**
   * The eturn a textual description of this Regression model.
   */
    ...
    abstract static class Predictor implements Serializable {
      ...
      protected Predictor(Element predictor, Instances miningSchema) throws Exception {
        ...
        if (m_miningSchemaAttIndex == -1) {
          throw new Exception(
              "[Predictor] unable to find matching attribute for " + "predictor " + m_name);
        }
          ...
          for (int i = 0; i < fields.getLength(); i++) {
            ...
            if (fieldRef.getNodeType() == Node.ELEMENT_NODE) {
              ...
              if (fieldName != null && fieldName.length() > 0) {
                ...
                if (!found) {
                  throw new Exception(
                      "[PredictorTerm] Unable to find field " + fieldName + " in mining schema!");
                }
              }
            }
          }
  ...
  public double[] distributionForInstance(Instance inst) throws Exception {
    ...
    for (int i = 0; i < incoming.length; i++) {
      if (i != m_miningSchema.getFieldsAsInstances().classIndex() && Utils.isMissingValue(
          incoming[i])) {
        hasMissing = true;
        break;
      }
