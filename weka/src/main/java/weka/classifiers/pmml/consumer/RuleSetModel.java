...
public class RuleSetModel extends PMMLClassifier {
    ...
    public SimpleRule(Element ruleE, MiningSchema miningSchema) throws Exception {
      ...
      if (miningSchema.getFieldsAsInstances().classAttribute().isNominal()) {
          ...
          if (child.getNodeType() == Node.ELEMENT_NODE) {
            ...
            if (tagName.equals("ScoreDistribution")) {
              TreeModel.ScoreDistribution newDist =
                  new TreeModel.ScoreDistribution((Element) child, miningSchema, m_recordCount);
        ...
        if (m_scoreDistributions.size() > 0
            && m_scoreDistributions.size() != miningSchema.getFieldsAsInstances()
            .classAttribute()
            .numValues()) {
  ...
  *
  ...
  RuleSetModel
  *
  @param dataDictionary the
  data dictionary
  to use
  *
  @param miningSchema the
  mining schema
  to use
  *@throws Exception if
  something goes
  ...
  */
  ...
  public RuleSetModel(Element model, Instances dataDictionary, MiningSchema miningSchema)
      throws Exception {
