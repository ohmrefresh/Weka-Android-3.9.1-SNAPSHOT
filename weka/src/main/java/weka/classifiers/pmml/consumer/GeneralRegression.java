...
public class GeneralRegression extends PMMLClassifier implements Serializable {
              ...
              if (p != null) {
                ...
                if (miningSchemaI.attribute(p.m_miningSchemaIndex).isNumeric()) {
                  ...
                  try {
                    ...
                    throw new Exception(
                        "[GeneralRegresion] unable to parse PPCell value: " + value);
                  }
                } else {
  ...
  private void readParamMatrix(Element model) throws Exception {
    ...
    // allocate space for the matrix
    m_paramMatrix =
        new PCell[(classAtt.isNumeric()) ? 1 : classAtt.numValues()][m_parameterList.size()];
      ...
      if (pcell.getNodeType() == Node.ELEMENT_NODE) {
        ...
        if (df != null && df.length() > 0) {
          try {
            ...
            throw new Exception(
                "[GeneralRegression] unable to parse df value " + df + " as an int from PCell");
          }
        }
  ...
  public double[] distributionForInstance(Instance inst) throws Exception {
    ...
    if (hasMissing) {
      if (!m_miningSchema.hasTargetMetaData()) {
        String message =
            "[GeneralRegression] WARNING: Instance to predict has missing value(s) but "
        ...
        if (cellEntry != null) {
          if ((p = getFactor(cellEntry.m_predictorName)) != null) {
            ...
            throw new Exception("[GeneralRegression] can't find predictor "
                + cellEntry.m_predictorName
                + " in either the list of factors or covariates");
          }
        }
      }
    }
   ...
   */
  private void computeResponses(double[] incomingInst, double[] incomingParamVector,
  ...
  enum LinkFunction {
    ...
    ODDSPOWER("oddspower") {
      double eval(double value, double offset, double trials, double distParam, double linkParam) {
        return (linkParam < 0.0 || linkParam > 0.0) ? (1.0 / (1.0 + Math.pow(
            1.0 + linkParam * (value + offset), (-1.0 / linkParam)))) * trials
            : (1.0 / (1.0 + Math.exp(-(value + offset)))) * trials;
      }
    },
    POWER("power") {
      double eval(double value, double offset, double trials, double distParam, double linkParam) {
        return (linkParam < 0.0 || linkParam > 0.0) ? Math.pow(value + offset, (1.0 / linkParam))
            * trials : Math.exp(value + offset) * trials;
      }
    },
    PROBIT("probit") {
      double eval(double value, double offset, double trials, double distParam, double linkParam) {
     ...
     * @return the result of the link function
     */
    abstract double eval(double value, double offset, double trials, double distParam,
        double linkParam);
  ...
  protected void printPPMatrix(StringBuffer buff) {
    ...
    buff.append(PMMLUtils.pad("Predictor", " ",
        (maxParamWidth + (maxAttWidth * 2 + 2)) - "Predictor".length(), true));
    buff.append(
  ...
  protected void printParameterMatrix(StringBuffer buff) {
    ...
    buff.append(PMMLUtils.pad(miningSchemaI.classAttribute().name(), " ",
        maxClassWidth + maxParamWidth + 2 - miningSchemaI.classAttribute().name().length(), false));
    ...
    for (int i = 0; i < m_paramMatrix.length; i++) {
      ...
      String cVal =
          (miningSchemaI.classAttribute().isNominal() || miningSchemaI.classAttribute().isString())
      ...
      for (int j = 0; j < m_parameterList.size(); j++) {
        ...
        if (p != null) {
          ...
          buff.append(
              PMMLUtils.pad(label, " ", maxClassWidth + maxParamWidth + 2 - label.length(), true));
