...
/**
 * <!-- globalinfo-start -->
 * Class for doing classification using regression methods. Class is binarized and one regression
 * model is built for each class value. For more information, see, for example<br/>
 * <br/>
...
public class ClassificationViaRegression extends SingleClassifierEnhancer
  ...
  public TechnicalInformation getTechnicalInformation() {
    ...
    result.setValue(Field.AUTHOR,
        "E. Frank and Y. Wang and S. Inglis and G. Holmes and I.H. Witten");
    ...
    for (int i = 0; i < inst.numClasses(); i++) {
      ...
      if (Utils.isMissingValue(probs[i])) {
        throw new UnassignedClassException(
            "ClassificationViaRegression: base learner predicted missing value.");
      }
      if (probs[i] > 1) {
        probs[i] = 1;
      }
