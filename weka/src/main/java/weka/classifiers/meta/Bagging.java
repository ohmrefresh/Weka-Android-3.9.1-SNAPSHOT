...
/**
 * <!-- globalinfo-start -->
 * Class for bagging a classifier to reduce variance. Can do classification and regression depending
 * on the base learner. <br/>
 * <br/>
...
public class Bagging extends RandomizableParallelIteratedSingleClassifierEnhancer
    ...
    PartitionGenerator, Aggregateable<Bagging> {
  ...
  @Override public void setOptions(String[] options) throws Exception {
    ...
    setOutputOutOfBagComplexityStatistics(
        Utils.getFlag("output-out-of-bag-complexity-statistics", options));
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option(
        "\tSize of each bag, as a percentage of the\n" + "\ttraining set size. (default 100)", "P",
        1, "-P"));
    newVector.addElement(new Option("\tCalculate the out of bag error.", "O", 0, "-O"));
    newVector.addElement(
        new Option("\tWhether to store out of bag predictions in internal evaluation object.",
    ...
    newVector.addElement(
        new Option("\tRepresent copies of instances using weights rather than explicitly.",
            "represent-copies-using-weights", 0, "-represent-copies-using-weights"));
    newVector.addElement(
        new Option("\tPrint the individual classifiers in the output", "print", 0, "-print"));
  ...
  @Override public String getRevision() {
    return RevisionUtils.extract("$Revision: 12843 $");
  }
    ...
    if (getCalcOutOfBag()) {
      ...
      for (int i = 0; i < m_data.numInstances(); i++) {
        ...
        if (m_Numeric) {
          if (voteCount > 0) {
            votes[0] /= voteCount;
            m_OutOfBagEvaluationObject.evaluationForSingleInstance(votes, m_data.instance(i),
    ...
    if (m_CalcOutOfBag) {
      ...
      if (bagSize < m_data.numInstances()) {
        bagData = m_data.resampleWithWeights(r,
            false); // Need to turn off representation using weights in this case.
        bagData.randomize(r);
