...
public class FilteredClassifier extends SingleClassifierEnhancer
  ...
  /**
   * Flag that can be set to true if class attribute is not to be checked for modifications by the
   * filer.
   */
  ...
  /**
   * Use this method to determine whether classifier checks whether class attribute has been
   * modified by filter.
   */
  ...
  protected Instances setUp(Instances data) throws Exception {
    ...
    if ((!classAttribute.equals(data.classAttribute()))
      ...
      throw new IllegalArgumentException(
          "Cannot proceed: " + getFilterSpec() + " has modified the class attribute!");
    }
    // t.stop();
   ...
   * @throws Exception if this iteration fails for unexpected reasons
   */
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option("\tFull class name of filter to use, followed\n"
        + "\tby filter options.\n"
        + "\teg: \"weka.filters.unsupervised.attribute.Remove -V -R 1,2\"", "F", 1,
        "-F <filter specification>"));
