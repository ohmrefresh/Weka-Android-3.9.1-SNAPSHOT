...
public class AdaBoostM1 extends RandomizableIteratedSingleClassifierEnhancer
  ...
  public boolean next() throws Exception {
    ...
    if ((m_UseResampling) || (!(m_Classifier instanceof WeightedInstancesHandler))) {
      ...
      if (m_Classifiers[m_NumIterationsPerformed] instanceof Randomizable) {
        ((Randomizable) m_Classifiers[m_NumIterationsPerformed]).setSeed(
            m_RandomInstance.nextInt());
      }
    ...
    if (m_Debug) {
      System.err.println(
          "\terror rate = " + epsilon + "  beta = " + m_Betas[m_NumIterationsPerformed]);
    }
  ...
  @Override public String toSource(String className) throws Exception {
    ...
    if (m_NumIterationsPerformed == 1) {
      ...
      for (int i = 0; i < m_NumIterationsPerformed; i++) {
        text.append(
            "    sums[(int) " + className + '_' + i + ".classify(i)] += " + m_Betas[i] + ";\n");
      }
