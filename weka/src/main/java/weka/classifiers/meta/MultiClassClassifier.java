...
/**
 * <!-- globalinfo-start -->
 * A metaclassifier for handling multi-class datasets with 2-class classifiers. This classifier is
 * also capable of applying error correcting output codes for increased accuracy.
 * <p/>
 * <!-- globalinfo-end -->
 *
 * <!-- options-start -->
...
public class MultiClassClassifier extends RandomizableSingleClassifierEnhancer
    ...
    if (numClassifiers <= 2) {
      ...
      switch (m_Method) {
        ...
        case METHOD_ERROR_RANDOM:
          code =
              new RandomCode(numClassifiers, (int) (numClassifiers * m_RandomWidthFactor), insts);
          break;
  ...
  public Enumeration<Option> listOptions() {
    ...
    vec.addElement(new Option("\tSets the method to use. Valid values are 0 (1-against-all),\n"
        ...
        "-M <num>"));
    vec.addElement(
        new Option("\tSets the multiplier when using random codes. (default 2.0)", "R", 1,
            "-R <num>"));
    vec.addElement(
        new Option("\tUse pairwise coupling (only has an effect for 1-against1)", "P", 0, "-P"));
    vec.addElement(
        new Option("\tUse log loss decoding for random and exhaustive codes", "L", 0, "-L"));
