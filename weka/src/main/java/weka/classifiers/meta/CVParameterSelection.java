 ...
 * <br/>
 * R. Kohavi (1995). Wrappers for Performance Enhancement and Oblivious Decision Graphs. Department
...
public class CVParameterSelection extends RandomizableSingleClassifierEnhancer
  ...
  protected String[] createOptions() {
    ...
    for (int i = 0; i < m_CVParams.size(); i++) {
      CVParameter cvParam = m_CVParams.elementAt(i);
      ...
      if (cvParam.m_AddAtEnd) {
        options[--end] = "" + ((cvParam.m_RoundParam || isInt) ? Utils.doubleToString(paramValue, 4)
            : cvParam.m_ParamValue);
        //Utils.doubleToString(paramValue,4);
        options[--end] = "-" + cvParam.m_ParamChar;
      } else {
        options[start++] = "-" + cvParam.m_ParamChar;
        options[start++] =
            "" + ((cvParam.m_RoundParam || isInt) ? Utils.doubleToString(paramValue, 4)
                : cvParam.m_ParamValue);
        //+ Utils.doubleToString(paramValue,4);
      }
    }
  ...
  protected void findParamsByCrossValidation(int depth, Instances trainData, Random random)
    ...
    if (depth < m_CVParams.size()) {
      CVParameter cvParam = m_CVParams.elementAt(depth);
      ...
      for (cvParam.m_ParamValue = cvParam.m_Lower; cvParam.m_ParamValue <= upper;
  ...
  public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tNumber of folds used for cross validation (default 10).", "X", 1,
            "-X <number of folds>"));
    ...
    public CVParameter(String param) throws Exception {
      ...
      if (parts[2].equals("A")) {
        ...
        try {
          ...
          throw new Exception(
              "CVParameter " + param + ": Upper bound must be numeric, or 'A' or 'N'");
        }
      }
  ...
  public String toString() {
    ...
    String result =
        "Cross-validated Parameter selection.\n" + "Classifier: " + m_Classifier.getClass()
            .getName() + "\n";
    try {
      for (int i = 0; i < m_CVParams.size(); i++) {
        CVParameter cvParam = m_CVParams.elementAt(i);
