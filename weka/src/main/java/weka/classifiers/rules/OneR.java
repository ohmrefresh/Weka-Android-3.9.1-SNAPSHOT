...
public class OneR extends AbstractClassifier implements TechnicalInformationHandler, Sourcable {
  ...
  @Override public String toSource(String className) throws Exception {
    ...
    if (m_ZeroR != null) {
      ...
      result.append("    // chosen attribute: "
          + m_rule.m_attr.name()
          + " ("
          + m_rule.m_attr.index()
          + ")\n");
      ...
      result.append("    double[] classifications = new double[]{" + Utils.arrayToString(
          m_rule.m_classifications) + "};");
      result.append(" // ");
      ...
      if (m_rule.m_attr.isNominal()) {
        for (i = 0; i < m_rule.m_attr.numValues(); i++) {
          ...
          result.append("if (((String) i["
              + m_rule.m_attr.index()
              + "]).equals(\""
              + m_rule.m_attr.value(i)
              + "\"))\n");
          result.append("      v = "
              + i
              + "; // "
        ...
        result.append(
            "           ((Double) i[" + m_rule.m_attr.index() + "]) >= breakpoints[(int) v]) {\n");
        result.append("      v++;\n");
        result.append("    }\n");
      }
