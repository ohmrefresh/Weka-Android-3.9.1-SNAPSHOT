 ...
 */
public class DecisionTable extends AbstractClassifier
  ...
  protected double estimatePerformance(BitSet feature_set, int num_atts) throws Exception {
    ...
    m_entries =
        new Hashtable<DecisionTableHashKey, double[]>((int) (m_theInstances.numInstances() * 1.5));
  ...
  @Override public void buildClassifier(Instances data) throws Exception {
    ...
    if (m_theInstances.classAttribute().isNumeric()) {
      ...
      ((weka.filters.unsupervised.attribute.Discretize) m_disTransform).setAttributeIndices(
          rangeList);
    } else {
    ...
    // create hash table
    m_entries =
        new Hashtable<DecisionTableHashKey, double[]>((int) (m_dtInstances.numInstances() * 1.5));
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(new Option(
        "\tPerformance evaluation measure to use for selecting attributes.\n"
            + "\t(Default = accuracy for discrete class and rmse for numeric class)", "E", 1,
        "-E <acc | rmse | mae | auc>"));
