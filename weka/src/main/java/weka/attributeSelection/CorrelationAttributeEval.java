 ...
 */
public class CorrelationAttributeEval extends ASEvaluation
  ...
  @Override public void buildEvaluator(Instances data) throws Exception {
    ...
    if (data.classAttribute().isNumeric()) {
      ...
      double[][] binarizedClasses =
          new double[data.classAttribute().numValues()][data.numInstances()];
        ...
        for (Integer i : nominalIndexes) {
          ...
          for (int j = 0; j < data.attribute(i).numValues(); j++) {
            ...
            for (int k = 0; k < numClasses; k++) {
              ...
              corr =
                  Utils.correlation(nomAtts[i][j], binarizedClasses[k], binarizedClasses[k].length);
