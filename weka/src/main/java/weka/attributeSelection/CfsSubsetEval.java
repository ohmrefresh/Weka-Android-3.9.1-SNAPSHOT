 ...
 * <pre>
 * -Z
 *  Precompute the full correlation matrix at the outset, rather than compute correlations lazily
 * (as needed) during the search. Use this in conjuction with parallel processing in order to speed
 * up a backward search.
 * </pre>
 *
 * <pre>
 * -P &lt;int&gt;
 ...
 * @see Discretize
 */
public class CfsSubsetEval extends ASEvaluation
   ...
   * -Z
   *  Precompute the full correlation matrix at the outset, rather than compute correlations lazily
   * (as needed) during the search. Use this in conjuction with parallel processing in order to
   * speed up a backward search.
   * </pre>
   *
   * <pre>
   * -P &lt;int&gt;
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\t" + poolSizeTipText() + " (default 1)\n", "P", 1, "-P <int>"));
    newVector.addElement(
        new Option("\t" + numThreadsTipText() + " (default 1)\n", "E", 1, "-E <int>"));
    ...
    if (m_preComputeCorrelationMatrix && m_poolSize > 1) {
      ...
      for (int i = 0; i < m_corr_matrix.length; i++) {
        for (int j = 0; j < m_corr_matrix[i].length; j++) {
          count++;
          if (count == numEntriesPerThread || (i == m_corr_matrix.length - 1
              && j == m_corr_matrix[i].length - 1)) {
            ...
            Future<Void> future = m_pool.submit(new Callable<Void>() {
              @Override public Void call() throws Exception {
                ...
                for (int i = sR; i <= eR; i++) {
                  for (int j = (i == sR ? sC : 0); j < (i == eR ? eC : m_corr_matrix[i].length);
                      j++) {
                ...
                if (m_debug) {
                  System.err.println(
                      "Percentage of correlation matrix computed: " + Utils.doubleToString(
  ...
  private double num_nom2(int att1, int att2) {
    ...
    double diff1, diff2;
    double r = 0.0, rr;
    int nx = (!m_missingSeparate) ? m_trainInstances.attribute(att1).numValues()
  ...
  private double nom_nom(int att1, int att2) {
    ...
    int ny = (!m_missingSeparate) ? m_trainInstances.attribute(att2).numValues()
   ...
   */
  @Override public int[] postProcess(int[] attributeSet) throws Exception {

    if (m_debug) {
      System.err.println(
          "Percentage of correlation matrix computed " + "over the search: " + Utils.doubleToString(
              ((double) m_numFilled.get() / m_numEntries * 100.0), 2) + "%");
    }
