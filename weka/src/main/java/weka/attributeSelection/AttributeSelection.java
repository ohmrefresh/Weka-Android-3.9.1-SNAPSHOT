...
public class AttributeSelection implements Serializable, RevisionHandler {
  ...
  public static void main(String[] args) {
    try {
      if (args.length == 0) {
        throw new Exception(
            "The first argument must be the name of an " + "attribute/subset evaluator");
      }
   ...
   * @throws Exception if no training file is set
   */
   ...
   * @return the results of attribute selection as a String
   * @throws Exception if incorrect options are supplied
   */
   ...
   * @throws Exception if there is a problem during selection
   */
  public void SelectAttributes(Instances data) throws Exception {
    ...
    if (m_ASEvaluator instanceof SubsetEvaluator && m_searchMethod instanceof Ranker) {
      throw new Exception(
          m_ASEvaluator.getClass().getName() + " must use a search method other than Ranker");
    }
    ...
    if ((m_searchMethod instanceof RankedOutputSearch) && m_doRank == true) {
      ...
      for (int i = 0; i < m_numToSelect; i++) {
        double precision =
            (Math.abs(m_attributeRanking[i][1]) - (int) (Math.abs(m_attributeRanking[i][1])));
        ...
        } else if ((Math.abs((Math.log(Math.abs(m_attributeRanking[i][1])) / Math.log(10))) + 1)
            > w_p) {
  ...
  private String printSelectionResults() {
    ...
    if (!(m_ASEvaluator instanceof UnsupervisedSubsetEvaluator)
      ...
      text.append((m_trainInstances.classIndex() + 1) + " " + m_trainInstances.attribute(
          m_trainInstances.classIndex()).name() + "):\n");
    } else {
   ...
   * @throws Exception if no attribute selection has been performed.
   */
    ...
    if ((m_searchMethod instanceof RankedOutputSearch) && (m_doRank == true)) {
      ...
      for (int element : s) {
        if (m_rankResults[1][element] > 0) {
          ...
          /*
           * Math. abs(
           */m_rankResults[0][element]/* ) */, 6, 3) + " +-" + Utils.doubleToString(
              m_rankResults[2][element], 6, 3) + "   " + Utils.doubleToString(
              m_rankResults[1][element], fieldWidth + 2, 1) + " +-" + Utils.doubleToString(
              m_rankResults[3][element], 5, 2) + "  " + Utils.doubleToString((element + 1),
              fieldWidth, 0) + " " + m_trainInstances.attribute(element).name() + "\n");
        }
      }
    } else {
      ...
      for (int i = 0; i < m_subsetResults.length; i++) {
        if ((m_ASEvaluator instanceof UnsupervisedSubsetEvaluator) || (i
            != m_trainInstances.classIndex())) {
          CvString.append(
              Utils.doubleToString(m_subsetResults[i], 12, 0) + "(" + Utils.doubleToString(
                  (m_subsetResults[i] / m_numFolds * 100.0), 3, 0) + " %)  " + Utils.doubleToString(
                  (i + 1), fieldWidth, 0) + " " + m_trainInstances.attribute(i).name() + "\n");
        }
      }
    }
   ...
   * @throws Exception if a ranking has not been produced
   */
   ...
   * @throws Exception if the instances can't be reduced
   */
   ...
   * @throws Exception if the instance can't be reduced
   */
