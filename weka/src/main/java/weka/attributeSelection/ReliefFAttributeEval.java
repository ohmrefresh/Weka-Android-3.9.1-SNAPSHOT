 ...
 */
public class ReliefFAttributeEval extends ASEvaluation
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tSeed for randomly sampling instances.\n" + "\t(Default = 1)", "D", 1,
            "-D <seed>"));
  ...
  @Override public void buildEvaluator(Instances data) throws Exception {
    ...
    m_numericClass = m_trainInstances.attribute(m_classIndex).isNumeric();
  ...
  @Override public int[] postProcess(int[] attributeSet) {
  ...
  private void updateWeightsNumericClass(int instNum) {
    ...
    for (i = 0; i < m_stored[0]; i++) {
      ...
      double temp_diffP_diffA_givNearest =
          difference(m_classIndex, inst.value(m_classIndex), cmp.value(m_classIndex));
      // now the attributes
  ...
  private void updateWeightsDiscreteClass(int instNum) {
    ...
    for (j = 0, temp_diff = 0.0; j < m_stored[cl]; j++) {
      Instance cmp;
      cmp = (m_weightByDistance) ? m_trainInstances.instance(
