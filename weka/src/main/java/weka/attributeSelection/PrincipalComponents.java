...
public class PrincipalComponents extends UnsupervisedAttributeEvaluator
  ...
  private Instances setOutputFormat() throws Exception {
    ...
    for (int i = m_numAttribs - 1; i >= 0; i--) {
      ...
      int num_attrs =
          (m_maxAttrsInName > 0) ? Math.min(m_numAttribs, m_maxAttrsInName) : m_numAttribs;
      ...
      for (int j = 0; j < num_attrs; j++) {
        ...
        attName.append(
            Utils.doubleToString(coeff_value, 5, 3) + m_trainInstances.attribute(coeff_inds[j])
                .name());
      }
    ...
    Instances outputFormat =
        new Instances(m_trainInstances.relationName() + "_principal components", attributes, 0);
  ...
  private Instances setOutputFormatOriginal() throws Exception {
    ...
    Instances outputFormat =
        new Instances(m_trainHeader.relationName() + "->PC->original space", attributes, 0);
   ...
   * determined.
   */
  @Override public Instances transformedHeader() throws Exception {
    if (m_eigenvalues == null) {
   ...
   */
  @Override public Instances transformedData(Instances data) throws Exception {
    if (m_eigenvalues == null) {
      throw new Exception("Principal components hasn't been built yet");
    }
   ...
   */
  @Override public Instance convertInstance(Instance instance) throws Exception {
   ...
   */
  @Override public double evaluateAttribute(int att) throws Exception {
    if (m_eigenvalues == null) {
      throw new Exception("Principal components hasn't been built yet!");
    }
