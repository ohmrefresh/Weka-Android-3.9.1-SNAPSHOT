...
public class WrapperSubsetEval extends ASEvaluation
  ...
  static {
    ...
    TAGS_EVALUATION[2] =
        new Tag(EVAL_RMSE, "rmse", "RMSE (of the class probabilities for discrete class)");
    TAGS_EVALUATION[3] =
        new Tag(EVAL_MAE, "mae", "MAE (of the class probabilities for discrete class)");
    TAGS_EVALUATION[4] = new Tag(EVAL_FMEASURE, "f-meas", "F-measure (discrete class only)");
    TAGS_EVALUATION[5] =
        new Tag(EVAL_AUC, "auc", "AUC (area under the ROC curve - discrete class only)");
    TAGS_EVALUATION[6] = new Tag(EVAL_AUPRC, "auprc",
        "AUPRC (area under the precision-recall curve - discrete class only)");
    TAGS_EVALUATION[7] =
        new Tag(EVAL_CORRELATION, "corr-coeff", "Correlation coefficient - numeric class only");
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    newVector.addElement(
        new Option("\tSeed for cross validation accuracy testimation.\n" + "\t(default = 1)", "R",
            1, "-R <seed>"));
  ...
  @Override public double evaluateSubset(BitSet subset) throws Exception {
      ...
      switch (m_evaluationMeasure.getID()) {
        ...
        default:
          ...
          if (pluginMetric instanceof InformationRetrievalEvaluationMetric) {
            if (m_IRClassVal < 0) {
              repError[i] =
                  ((InformationRetrievalEvaluationMetric) pluginMetric).getClassWeightedAverageStatistic(
                      statName);
            } else {
              repError[i] =
                  ((InformationRetrievalEvaluationMetric) pluginMetric).getStatistic(statName,
                      m_IRClassVal);
            }
          } else {
    ...
    switch (m_evaluationMeasure.getID()) {
      ...
      case EVAL_RMSE:
      case EVAL_MAE:
        if (m_trainInstances.classAttribute().isNominal() && (m_evaluationMeasure.getID()
            == EVAL_DEFAULT || m_evaluationMeasure.getID() == EVAL_ACCURACY)) {
  ...
  private boolean repeat(double[] repError, int entries) {
    ...
    return (variance / mean) > m_threshold;
  }
  ...
  @Override public String toString() {
    ...
    if (m_trainInstances == null) {
      ...
      switch (m_evaluationMeasure.getID()) {
        ...
        case EVAL_FMEASURE:
          text.append(
              "\tSubset evaluation: F-measure " + (m_IRClassVal >= 0 ? IRClassL : "") + "\n");
          break;
        case EVAL_AUC:
        ...
        case EVAL_AUPRC:
          text.append(
              "\tSubset evaluation: area under the precision-recall curve " + (m_IRClassVal >= 0
                  ? IRClassL : "") + "\n");
          break;
        case EVAL_CORRELATION:
