...
public class FilteredAssociator extends SingleAssociatorEnhancer
  ...
  public FilteredAssociator() {
    ...
    ((MultiFilter) m_Filter).setFilters(
        new Filter[] { new weka.filters.unsupervised.attribute.ReplaceMissingValues() });
    m_ClassIndex = -1;
  }
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    result.addElement(new Option("\tFull class name of filter to use, followed\n"
        ...
        + "\teg: \"weka.filters.unsupervised.attribute.Remove -V -R 1,2\"\n"
        + "\t(default: weka.filters.MultiFilter with\n"
        + "\tweka.filters.unsupervised.attribute.ReplaceMissingValues)", "F", 1,
        "-F <filter specification>"));

    result.addElement(new Option("\tThe class index.\n" + "\t(default: -1, i.e. unset)", "c", 1,
        "-c <the class index>"));
