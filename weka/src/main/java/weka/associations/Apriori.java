...
public class Apriori extends AbstractAssociator
  ...
  public static final Tag[] TAGS_SELECTION = {
      new Tag(CONFIDENCE, "Confidence"), new Tag(LIFT, "Lift"), new Tag(LEVERAGE, "Leverage"),
   ...
   */
  @Override public ArrayList<Object>[] mineCARs(Instances data) throws Exception {
   ...
   * @throws Exception if rules can't be built successfully
   */
  @SuppressWarnings("unchecked") @Override public void buildAssociations(Instances instances)
      throws Exception {
    ...
    double lowerBoundMinSupportToUse =
        (m_lowerBoundMinSupport * instances.numInstances() < 1.0) ? 1.0 / instances.numInstances()
    ...
    if (m_car && m_numRules == Integer.MAX_VALUE) {
      ...
      m_minSupport =
          (m_minSupport < lowerBoundMinSupportToUse) ? lowerBoundMinSupportToUse : m_minSupport;
    }
    ...
    do {
      ...
      m_cycles++;
    } while ((m_allTheRules[0].size() < m_numRules) && (Utils.grOrEq(m_minSupport,
  ...
  private void findRulesBruteForce() throws Exception {
    ...
    for (int j = 1; j < m_Ls.size(); j++) {
      ...
      while (enumItemSets.hasMoreElements()) {
        ...
        rules =
            currentItemSet.generateRulesBruteForce(m_minMetric, m_metricType, m_hashtables, j + 1,
  ...
  @Override public void setOptions(String[] options) throws Exception {
    ...
    resetOptions();
    String numRulesString = Utils.getOption('N', options), minConfidenceString =
        Utils.getOption('C', options), deltaString = Utils.getOption('D', options),
        maxSupportString = Utils.getOption('U', options), minSupportString =
        Utils.getOption('M', options), significanceLevelString = Utils.getOption('S', options),
        classIndexString = Utils.getOption('c', options), toStringDelimitersString =
        Utils.getOption('B', options);
  ...
  @Override public String toString() {
    ...
    text.append("\nApriori\n=======\n\n");
    text.append(
        "Minimum support: " + Utils.doubleToString(m_minSupport, 2) + " (" + ((int) (m_minSupport
            * m_instances.numInstances() + 0.5)) + " instances)" + '\n');
    ...
    if (!m_car) {
      ...
      for (int i = 0; i < m_allTheRules[0].size(); i++) {
        ...
        String n =
            Utils.doubleToString((double) i + 1, (int) (Math.log(m_numRules) / Math.log(10) + 1),
                0);

        String premise = ((AprioriItemSet) m_allTheRules[0].get(i)).toString(m_instances, odc, idc);
        String consequence =
            ((AprioriItemSet) m_allTheRules[1].get(i)).toString(m_instances, odc, idc);
        ...
        int leverageT =
            (int) (((Double) m_allTheRules[4].get(i)).doubleValue() * m_instances.numInstances());
   ...
   */
  @Override public Instances getInstancesNoClass() {
   ...
   */
  @Override public Instances getInstancesOnlyClass() {
  ...
  @Override public Enumeration<Option> listOptions() {
    ...
    String string1 = "\tThe required number of rules. (default = " + m_numRules + ")", string2 =
        "\tThe minimum confidence of a rule. (default = " + m_minMetric + ")", string3 =
        "\tThe delta by which the minimum support is decreased in\n", string4 =
        "\teach iteration. (default = " + m_delta + ")", string5 =
        "\tThe lower bound for the minimum support. (default = " + m_lowerBoundMinSupport + ")",
        string6 = "\tIf used, rules are tested for significance at\n", string7 =
        "\tthe given level. Slower. (default = no significance testing)", string8 =
        "\tIf set the itemsets found are also output. (default = no)", string9 =
        "\tIf set class association rules are mined. (default = no)", string10 =
        "\tThe class index. (default = last)", stringType =
        "\tThe metric type by which to rank rules. (default = " + "confidence)",
        stringZeroAsMissing =
            "\tTreat zero (i.e. first value of nominal attributes) as " + "missing",
    ...
    newVector.add(new Option(stringType, "T", 1,
        "-T <0=confidence | 1=lift | " + "2=leverage | 3=Conviction>"));
    ...
    newVector.add(
        new Option("\tRemove columns that contain " + "all missing values (default = no)", "R", 0,
            "-R"));
   ...
   */
  @Override public void setClassIndex(int index) {
    ...
    if (m_allTheRules != null && m_allTheRules.length > 3) {
      for (int i = 0; i < m_allTheRules[0].size(); i++) {
        ...
        DefaultAssociationRule newRule =
            new DefaultAssociationRule(premise, consequence, metric, premiseSupport,
                consequenceSupport, totalSupport, totalTrans);
